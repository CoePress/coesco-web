...
export class PollingService {
  ...
  private async processMTConnectData(
    data: any,
    machineId: string
  ): Promise<MachineStateWithoutId> {
    const streams = data.MTConnectStreams?.Streams?.DeviceStream;
    if (!streams) return this.createOfflineState(machineId);

    const path = this.findComponent(streams, "Path");
    if (!path) return this.createOfflineState(machineId);

    return {
      machineId,
      state: this.determineState(path),
      controllerMode: path.Events?.ControllerMode?._text || "",
      executionStatus: path.Events?.Execution?._text || "",
      isAxisActive: await this.checkAxisActivity(streams),
      isFeedActive: await this.checkFeedActivity(streams),
      isSpindleActive: await this.checkSpindleActivity(streams),
      program: path.Events?.Program?._text || null,
      tool: path.Events?.ToolNumber?._text || null,
      startedAt: new Date(),
      createdAt: new Date(),
      updatedAt: new Date(),
    };
  ...
  private async processFanucData(
    data: any,
    machineId: string
  ): Promise<MachineStateWithoutId> {
    const machine = data.MachineData || {};
    return {
      machineId,
      state: this.determineFanucState(machine),
      controllerMode: machine.mode?._text || "",
      executionStatus: machine.status?._text || "",
      isAxisActive: await this.checkFanucAxisActivity(machine),
      isFeedActive: Boolean(machine.feedrate?._text),
      isSpindleActive: Boolean(machine.spindle?._text),
      program: machine.program?._text || null,
      tool: machine.tool?._text || null,
      startedAt: new Date(),
      createdAt: new Date(),
      updatedAt: new Date(),
    };
  ...
  private determineState(path: any): string {
    const execution = path.Events?.Execution?._text || "UNKNOWN";
    const mode = path.Events?.ControllerMode?._text;

    if (execution === "ACTIVE" || execution === "EXECUTING") return "RUNNING";
    if (execution === "READY" && mode === "AUTOMATIC") return "IDLE";
    if (execution === "READY" && mode === "MANUAL") return "MANUAL";
    if (execution === "STOPPED") return "STOPPED";
    if (execution === "INTERRUPTED" || execution === "FEED_HOLD") return "HELD";
    return "UNKNOWN";
  }

  private determineFanucState(machine: any): string {
    const status = machine.status?._text || "UNKNOWN";
    const mode = machine.mode?._text;

    if (status === "RUNNING") return "RUNNING";
    if (status === "IDLE" && mode === "AUTO") return "IDLE";
    if (status === "IDLE" && mode === "MDI") return "MANUAL";
