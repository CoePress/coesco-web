...
echo "Step 1: Pruning monorepo..."
turbo prune @coesco/server --out-dir deploy

# 2. Send ONLY the source files to production
echo "Step 2: Sending files to production..."
cd deploy
scp -r . ${PRODUCTION_HOST}:${PRODUCTION_PATH}/

# 3-7. Execute remaining steps on production server
echo "Step 3-7: Installing dependencies, generating Prisma client, building, and restarting service on production..."
...
echo "ssh ${PRODUCTION_HOST} 'journalctl -f --output=cat -u server.service'"
