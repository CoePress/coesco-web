  ...
  private static readonly ENGINEERING_DEFAULTS: EngineeringDefaults = {
    ...
    'material-specs': {
      ...
      'materialSpecs.material.minBendRadius': {
        value: (data: PerformanceData) => {
          const thickness = parseFloat(data?.common?.material?.materialThickness?.toString() || '0');
          const yieldStrength = parseFloat(data?.common?.material?.maxYieldStrength?.toString() || '50000');
    ...
    'str-utility': {
      ...
      'strUtility.straightener.feedRate': {
        value: (data: PerformanceData) => {
          const avgSpm = parseFloat(data?.common?.feedRates?.average?.spm?.toString() || '30');
          const avgLength = parseFloat(data?.common?.feedRates?.average?.length?.toString() || '12');
      ...
      'strUtility.straightener.horsepower': {
        value: (data: PerformanceData) => {
          const thickness = parseFloat(data?.common?.material?.materialThickness?.toString() || '0.125');
          const width = parseFloat(data?.common?.material?.coilWidth?.toString() || '12');
          const yieldStrength = parseFloat(data?.common?.material?.maxYieldStrength?.toString() || '50000');
    ...
    'reel-drive': {
      'reelDrive.reel.motorization.driveHorsepower': {
        value: (data: PerformanceData) => {
          const coilWeight = parseFloat(data?.common?.coil?.maxCoilWeight?.toString() || '5000');
          const speed = parseFloat(data?.reelDrive?.reel?.motorization?.speed?.toString() || '100');
    ...
    'shear': {
      'shear.shear.hydraulic.pressure': {
        value: (data: PerformanceData) => {
          const thickness = parseFloat(data?.common?.material?.materialThickness?.toString() || '0.125');
          const tensileStrength = parseFloat(data?.common?.material?.maxTensileStrength?.toString() ||
  ...
  public static getValidationAwareAutofill(
    ...
    for (const tabName of visibleTabNames) {
      ...
      for (const [fieldPath, defaultConfig] of Object.entries(tabDefaults)) {
        ...
        if (this.hasMeaningfulValue(currentValue)) continue;
