...
async function cleanupOrphanedRepositories(models: any) {
  const directory = path.resolve(__dirname, "../repositories");
  ...
  const currentModelNames = new Set(models.map((m: any) => toKebabCase(m.name)));
  const files = fs.readdirSync(directory);

  for (const file of files) {
    // Skip base repository, index, and non-.repository.ts files
    if (file === "_base.repository.ts" || file === "index.ts" || !file.endsWith(".repository.ts")) {
      continue;
    }

    const modelName = file.replace(".repository.ts", "");

    if (!currentModelNames.has(modelName)) {
      const filePath = path.resolve(directory, file);
      fs.unlinkSync(filePath);
      logger.info(`Deleted orphaned repository: ${file}`);
    }
  }
}

async function cleanupOrphanedTypes(models: any, enums: any) {
  const srcDir = path.resolve(__dirname, "../../../../packages/types/src");
  ...
  const currentNames = new Set([
    ...models.map((m: any) => toKebabCase(m.name)),
    ...enums.map((e: any) => toKebabCase(e.name)),
  ]);

  const files = fs.readdirSync(srcDir);

  for (const file of files) {
    // Skip special files and non-.ts files
    if (
      file === "index.ts"
      || file === "_auto-generated.ts"
      || file === "custom"
      || !file.endsWith(".ts")
    ) {
      continue;
    ...
    // Check if this is an auto-generated file by reading its first line
    const filePath = path.resolve(srcDir, file);
    const content = fs.readFileSync(filePath, "utf-8");
    const isAutoGenerated = content.startsWith("// Auto-generated from Prisma schema");

    if (isAutoGenerated && !currentNames.has(typeName)) {
      fs.unlinkSync(filePath);
      logger.info(`Deleted orphaned type file: ${file}`);
    }
  }
}
...
async function main() {
  ...
  await cleanupOrphanedRepositories(models);
  await cleanupOrphanedTypes(models, enums);
