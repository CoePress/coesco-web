...
export class OptionCategoryService extends BaseService<OptionCategory> {
	...
	protected async validate(entity: OptionCategoryAttributes): Promise<void> {
		...
		if (!entity.multiple) throw new BadRequestError("multiple is required");
		if (!entity.mandatory) throw new BadRequestError("mandatory is required");
		if (!entity.standard) throw new BadRequestError("standard is required");
