import { read } from "fs";

export const DAYS_PER_WEEK_OPTIONS = [
  { value: "1", label: "1 Day" },
  { value: "2", label: "2 Days" },
  { value: "3", label: "3 Days" },
  { value: "4", label: "4 Days" },
  { value: "5", label: "5 Days" },
  { value: "6", label: "6 Days" },
  { value: "7", label: "7 Days" },
];

export const YES_NO_OPTIONS = [
  { value: "yes", label: "Yes" },
  { value: "no", label: "No" },
];

export const LINE_APPLICATION_OPTIONS = [
  { value: "Press Feed", label: "Press Feed" },
  { value: "Cut to Length", label: "Cut to Length" },
  { value: "Standalone", label: "Standalone" },
];

export const LINE_TYPE_OPTIONS = [
  { value: "Conventional", label: "Conventional" },
  { value: "Compact", label: "Compact" },
];

export const LINE_TYPE_STANDALONE_OPTIONS = [
  { value: "Feed", label: "Feed" },
  { value: "Straightener", label: "Straightener" },
  { value: "Reel-Motorized", label: "Reel-Motorized" },
  { value: "Reel-Pull Off", label: "Reel-Pull Off" },
  { value: "Straightener-Reel Combination", label: "Straightener-Reel Combination" },
  { value: "Other", label: "Other" },
  { value: "Feed-Shear", label: "Feed-Shear" },
  { value: "Threading Table", label: "Threading Table" },
];

export const PULL_THRU_OPTIONS = [
  { value: "yes", label: "Yes" },
  { value: "no", label: "No" },
];

export const COIL_LOADING_OPTIONS = [
  { value: "Operator Side", label: "Operator Side" },
  { value: "Non-Operator Side", label: "Non-Operator Side" },
];

export const MATERIAL_TYPE_OPTIONS = [
  { value: "Aluminum", label: "Aluminum" },
  { value: "Galvanized", label: "Galvanized" },
  { value: "HS Steel", label: "HS Steel" },
  { value: "Hot Rolled Steel", label: "Hot Rolled Steel" },
  { value: "Dual Phase", label: "Dual Phase" },
  { value: "Cold Rolled Steel", label: "Cold Rolled Steel" },
  { value: "Stainless Steel", label: "Stainless Steel" },
  { value: "Titanium", label: "Titanium" },
  { value: "Brass", label: "Brass" },
  { value: "Beryl Copper", label: "Beryl Copper" },
];

export const CONTROLS_LEVEL_OPTIONS = [
  { value: "Mini-Drive System", label: "Mini-Drive System" },
  { value: "Relay Machine", label: "Relay Machine" },
  { value: "SyncMaster", label: "SyncMaster" },
  { value: "IP Indexer Basic", label: "IP Indexer Basic" },
  { value: "Allen Bradley Basic", label: "Allen Bradley Basic" },
  { value: "SyncMaster Plus", label: "SyncMaster Plus" },
  { value: "IP Indexer Plus", label: "IP Indexer Plus" },
  { value: "Allen Bradley Plus", label: "Allen Bradley Plus" },
  { value: "Fully Automatic", label: "Fully Automatic" },
];

export const FEED_DIRECTION_OPTIONS = [
  { value: "left-to-right", label: "Left to Right" },
  { value: "right-to-left", label: "Right to Left" },
];

export const ROLL_TYPE_OPTIONS = [
  { value: "7 Roll Str. Backbend", label: "7 Roll Str. Backbend" },
  { value: "9 Roll Str. Backbend", label: "9 Roll Str. Backbend" },
  { value: "11 Roll Str. Backbend", label: "11 Roll Str. Backbend" },
];

export const BACKPLATE_TYPE_OPTIONS = [
  { value: "Standard Backplate", label: "Standard Backplate" },
  { value: "Full OD Backplate", label: "Full OD Backplate" },
];

export const TYPE_OF_LINE_OPTIONS = [
  { value: "Compact", label: "Compact" },
  { value: "Compact CTL", label: "Compact CTL" },
  { value: "Conventional", label: "Conventional" },
  { value: "Conventional CTL", label: "Conventional CTL" },
  { value: "Pull Through", label: "Pull Through" },
  { value: "Pull Through Compact", label: "Pull Through Compact" },
  { value: "Pull Through CTL", label: "Pull Through CTL" },
  { value: "Feed", label: "Feed" },
  { value: "Feed-Pull Through", label: "Feed-Pull Through" },
  { value: "Feed-Pull Through-Shear", label: "Feed-Pull Through-Shear" },
  { value: "Feed-Shear", label: "Feed-Shear" },
  { value: "Straightener", label: "Straightener" },
  { value: "Straightener-Reel Combination", label: "Straightener-Reel Combination" },
  { value: "Reel-Motorized", label: "Reel-Motorized" },
  { value: "Reel-Pull Off", label: "Reel-Pull Off" },
  { value: "Threading Table", label: "Threading Table" },
  { value: "Other", label: "Other" },
];

export const PASSLINE_OPTIONS = [
  { value: "37", label: "37\"" },
  { value: "39", label: "39\"" },
  { value: "40", label: "40\"" },
  { value: "40.5", label: "40.5\"" },
  { value: "41", label: "41\"" },
  { value: "41.5", label: "41.5\"" },
  { value: "42", label: "42\"" },
  { value: "43", label: "43\"" },
  { value: "43.625", label: "43.625\"" },
  { value: "44", label: "44\"" },
  { value: "45", label: "45\"" },
  { value: "45.5", label: "45.5\"" },
  { value: "46", label: "46\"" },
  { value: "46.5", label: "46.5\"" },
  { value: "47", label: "47\"" },
  { value: "47.4", label: "47.4\"" },
  { value: "47.5", label: "47.5\"" },
  { value: "48", label: "48\"" },
  { value: "48.5", label: "48.5\"" },
  { value: "49", label: "49\"" },
  { value: "49.5", label: "49.5\"" },
  { value: "50", label: "50\"" },
  { value: "50.5", label: "50.5\"" },
  { value: "50.75", label: "50.75\"" },
  { value: "51", label: "51\"" },
  { value: "51.5", label: "51.5\"" },
  { value: "51.75", label: "51.75\"" },
  { value: "52", label: "52\"" },
  { value: "52.25", label: "52.25\"" },
  { value: "52.5", label: "52.5\"" },
  { value: "53", label: "53\"" },
  { value: "54", label: "54\"" },
  { value: "54.5", label: "54.5\"" },
  { value: "54.75", label: "54.75\"" },
  { value: "55", label: "55\"" },
  { value: "55.25", label: "55.25\"" },
  { value: "55.5", label: "55.5\"" },
  { value: "55.75", label: "55.75\"" },
  { value: "56", label: "56\"" },
  { value: "56.5", label: "56.5\"" },
  { value: "57", label: "57\"" },
  { value: "58", label: "58\"" },
  { value: "58.25", label: "58.25\"" },
  { value: "59", label: "59\"" },
  { value: "59.5", label: "59.5\"" },
  { value: "60", label: "60\"" },
  { value: "60.5", label: "60.5\"" },
  { value: "61", label: "61\"" },
  { value: "62", label: "62\"" },
  { value: "62.5", label: "62.5\"" },
  { value: "63", label: "63\"" },
  { value: "64", label: "64\"" },
  { value: "64.5", label: "64.5\"" },
  { value: "65", label: "65\"" },
  { value: "66", label: "66\"" },
  { value: "66.5", label: "66.5\"" },
  { value: "67", label: "67\"" },
  { value: "70", label: "70\"" },
  { value: "72", label: "72\"" },
  { value: "75", label: "75\"" },
  { value: "76", label: "76\"" },
];

export const FEED_MODEL_OPTIONS = [
  { value: "CPRF-S1", label: "CPRF-S1" },
  { value: "CPRF-S2", label: "CPRF-S2" },
  { value: "CPRF-S2 PLUS", label: "CPRF-S2 PLUS" },
  { value: "CPRF-S3", label: "CPRF-S3" },
  { value: "CPRF-S3 PLUS", label: "CPRF-S3 PLUS" },
  { value: "CPRF-S4", label: "CPRF-S4" },
  { value: "CPRF-S4 PLUS", label: "CPRF-S4 PLUS" },
  { value: "CPRF-S5", label: "CPRF-S5" },
  { value: "CPRF-S6", label: "CPRF-S6" },
  { value: "CPRF-S7", label: "CPRF-S7" },
  { value: "CPRF-S8", label: "CPRF-S8" },
];

export const FEED_TYPE_OPTIONS = [
  { value: "Sigma 5", label: "Sigma 5" },
  { value: "Sigma 5 w/ Pull Through", label: "Sigma 5 w/ Pull Through" },
  { value: "Allen Bradley MPL - Feed", label: "Allen Bradley MPL - Feed" },
];

export const MACHINE_WIDTH_OPTIONS = [
  { value: "6", label: "6\"" },
  { value: "12", label: "12\"" },
  { value: "18", label: "18\"" },
  { value: "24", label: "24\"" },
  { value: "30", label: "30\"" },
  { value: "36", label: "36\"" },
  { value: "42", label: "42\"" },
  { value: "48", label: "48\"" },
  { value: "54", label: "54\"" },
  { value: "60", label: "60\"" },
  { value: "66", label: "66\"" },
  { value: "72", label: "72\"" },
  { value: "78", label: "78\"" },
];

export const PRESS_APPLICATION_OPTIONS = [
  { value: "blanking", label: "Blanking" },
  { value: "progressive", label: "Progressive" },
  { value: "transfer", label: "Transfer" },
  { value: "forming", label: "Forming" },
];

export const STRAIGHTENER_ROLLS_OPTIONS = [
  { value: "yes", label: "Yes" },
  { value: "no", label: "No" },
];

export const STR_FEED_RATE_OPTIONS = [
  { value: "42", label: "42 ft/min" },
  { value: "57", label: "57 ft/min" },
  { value: "60", label: "60 ft/min" },
  { value: "65", label: "65 ft/min" },
  { value: "69", label: "69 ft/min" },
  { value: "72", label: "72 ft/min" },
  { value: "73", label: "73 ft/min" },
  { value: "76", label: "76 ft/min" },
  { value: "79", label: "79 ft/min" },
  { value: "80", label: "80 ft/min" },
  { value: "84", label: "84 ft/min" },
  { value: "85", label: "85 ft/min" },
  { value: "92", label: "92 ft/min" },
  { value: "95", label: "95 ft/min" },
  { value: "96", label: "96 ft/min" },
  { value: "100", label: "100 ft/min" },
  { value: "101", label: "101 ft/min" },
  { value: "105", label: "105 ft/min" },
  { value: "115", label: "115 ft/min" },
  { value: "116", label: "116 ft/min" },
  { value: "120", label: "120 ft/min" },
  { value: "122", label: "122 ft/min" },
  { value: "140", label: "140 ft/min" },
  { value: "146", label: "146 ft/min" },
  { value: "150", label: "150 ft/min" },
  { value: "153", label: "153 ft/min" },
  { value: "157", label: "157 ft/min" },
  { value: "160", label: "160 ft/min" },
  { value: "165", label: "165 ft/min" },
  { value: "177", label: "177 ft/min" },
  { value: "180", label: "180 ft/min" },
  { value: "200", label: "200 ft/min" },
  { value: "224", label: "224 ft/min" },
];

export const FEED_ANGLE_OPTIONS = [
  { value: "180", label: "180°" },
  { value: "240", label: "240°" },
  { value: "120", label: "120°" },
  { value: "90", label: "90°" },
  { value: "60", label: "60°" },
];

export const STR_MODEL_OPTIONS = [
  { value: "CPPS-250", label: "CPPS-250" },
  { value: "CPPS-306", label: "CPPS-306" },
  { value: "CPPS-350", label: "CPPS-350" },
  { value: "CPPS-406", label: "CPPS-406" },
  { value: "CPPS-507", label: "CPPS-507" },
  { value: "SPGPS-810", label: "SPGPS-810" },
];

export const STR_WIDTH_OPTIONS = [
  { value: "6", label: "6\"" },
  { value: "12", label: "12\"" },
  { value: "18", label: "18\"" },
  { value: "24", label: "24\"" },
  { value: "30", label: "30\"" },
  { value: "36", label: "36\"" },
  { value: "42", label: "42\"" },
  { value: "48", label: "48\"" },
  { value: "54", label: "54\"" },
  { value: "60", label: "60\"" },
  { value: "66", label: "66\"" },
  { value: "72", label: "72\"" },
  { value: "78", label: "78\"" },
];

export const PAYOFF_OPTIONS = [
  { value: "top", label: "Top" },
  { value: "bottom", label: "Bottom" },
];

export const STR_HORSEPOWER_OPTIONS = [
  { value: "3", label: "3 HP" },
  { value: "5", label: "5 HP" },
  { value: "7.5", label: "7.5 HP" },
  { value: "10", label: "10 HP" },
  { value: "15", label: "15 HP" },
  { value: "20", label: "20 HP" },
  { value: "25", label: "25 HP" },
  { value: "30", label: "30 HP" },
  { value: "40", label: "40 HP" },
  { value: "50", label: "50 HP" },
  { value: "60", label: "60 HP" },
  { value: "75", label: "75 HP" },
  { value: "100", label: "100 HP" },
  { value: "125", label: "125 HP" },
];

export const SHEAR_TYPE_OPTIONS = [
  { value: "single-rake", label: "Single Rake" },
  { value: "bow-tie", label: "Bow Tie" },
];

export const REEL_STYLE_OPTIONS = [
  { value: "Single Ended", label: "Single Ended" },
  { value: "Double Ended", label: "Double Ended" },
];

export const REEL_MODEL_OPTIONS = [
  { value: "CPR-040", label: "CPR-040" },
  { value: "CPR-060", label: "CPR-060" },
  { value: "CPR-080", label: "CPR-080" },
  { value: "CPR-100", label: "CPR-100" },
  { value: "CPR-150", label: "CPR-150" },
  { value: "CPR-200", label: "CPR-200" },
  { value: "CPR-300", label: "CPR-300" },
  { value: "CPR-400", label: "CPR-400" },
  { value: "CPR-500", label: "CPR-500" },
  { value: "CPR-600", label: "CPR-600" },
];

export const REEL_HORSEPOWER_OPTIONS = [
  { value: "1", label: "1 HP" },
  { value: "2", label: "2 HP" },
  { value: "3", label: "3 HP" },
  { value: "5", label: "5 HP" },
  { value: "7.5", label: "7.5 HP" },
  { value: "10", label: "10 HP" },
  { value: "15", label: "15 HP" },
  { value: "20", label: "20 HP" },
  { value: "25", label: "25 HP" },
  { value: "30", label: "30 HP" },
];

export const REEL_WIDTH_OPTIONS = [
  { value: "12", label: "12" },
  { value: "18", label: "18" },
  { value: "24", label: "24" },
  { value: "30", label: "30" },
  { value: "36", label: "36" },
  { value: "42", label: "42" },
  { value: "48", label: "48" },
  { value: "54", label: "54" },
  { value: "60", label: "60" },
  { value: "66", label: "66" },
  { value: "72", label: "72" },
  { value: "78", label: "78" },
];

export const BACKPLATE_DIAMETER_OPTIONS = [
  { value: "23", label: "23" },
  { value: "27", label: "27" },
  { value: "72", label: "72" },
];

export const HYDRAULIC_THREADING_DRIVE_OPTIONS = [
  { value: "None", label: "None" },
  { value: "22 cu in (D-15125)", label: "22 cu in (D-15125)" },
  { value: "38 cu in (D-15125)", label: "38 cu in (D-15125)" },
  { value: "22 cu in (D-12689)", label: "22 cu in (D-12689)" },
  { value: "38 cu in (D-13374)", label: "38 cu in (D-13374)" },
  { value: "60 cu in (D-13374)", label: "60 cu in (D-13374)" },
  { value: "60 cu in (D-13382)", label: "60 cu in (D-13382)" },
];

export const HOLD_DOWN_ASSY_OPTIONS = [
  { value: "None", label: "None" },
  { value: "LD_STANDARD", label: "LD Standard" },
  { value: "LD_NARROW", label: "LD Narrow" },
  { value: "LD_MOTORIZED", label: "LD Motorized" },
  { value: "SD", label: "SD" },
  { value: "SD_MOTORIZED", label: "SD Motorized" },
  { value: "MD", label: "MD" },
  { value: "HD_Single", label: "HD Single" },
  { value: "HD_Dual", label: "HD Dual" },
  { value: "XD", label: "XD" },
  { value: "XXD", label: "XXD" },
];

export const CYLINDER_OPTIONS = [
  { value: "Air", label: "Air" },
  { value: "4in Air", label: "4in Air" },
  { value: "5in Air", label: "5in Air" },
  { value: "8in Air", label: "8in Air" },
  { value: "Hydraulic", label: "Hydraulic" },
];

export const BRAKE_MODEL_OPTIONS = [
  { value: "Single Stage", label: "Single Stage" },
  { value: "Double Stage", label: "Double Stage" },
  { value: "Triple Stage", label: "Triple Stage" },
  { value: "Failsafe - Single Stage", label: "Failsafe - Single Stage" },
  { value: "Failsafe - Double Stage", label: "Failsafe - Double Stage" },
];

export const BRAKE_QUANTITY_OPTIONS = [
  { value: "1", label: "1" },
  { value: "2", label: "2" },
  { value: "3", label: "3" },
];

// Reel model dependency mappings
export const REEL_MODEL_DEPENDENCIES = {
  "CPR-040": {
    widths: ["12", "18", "24", "30", "36"],
    backplateDiameters: ["23"],
    hydThreadingDrives: ["None"],
    holdDownAssys: ["None", "LD_STANDARD", "LD_NARROW"]
  },
  "CPR-060": {
    widths: ["18", "24", "30", "36"],
    backplateDiameters: ["23"],
    hydThreadingDrives: ["22 cu in (D-15125)", "38 cu in (D-15125)", "None"],
    holdDownAssys: ["None", "LD_STANDARD", "LD_NARROW", "LD_MOTORIZED"]
  },
  "CPR-080": {
    widths: ["18", "24", "30", "36"],
    backplateDiameters: ["23"],
    hydThreadingDrives: ["22 cu in (D-15125)", "38 cu in (D-15125)", "None"],
    holdDownAssys: ["None", "LD_STANDARD", "LD_NARROW", "LD_MOTORIZED"]
  },
  "CPR-100": {
    widths: ["24", "30", "36", "42", "48", "54", "60"],
    backplateDiameters: ["27", "72"],
    hydThreadingDrives: ["22 cu in (D-12689)", "38 cu in (D-13374)", "60 cu in (D-13374)", "60 cu in (D-13382)"],
    holdDownAssys: ["SD", "SD_MOTORIZED", "MD", "HD_Single", "HD_Dual", "XD", "XXD"]
  },
  "CPR-150": {
    widths: ["24", "30", "36", "42", "48", "54", "60"],
    backplateDiameters: ["27", "72"],
    hydThreadingDrives: ["22 cu in (D-12689)", "38 cu in (D-13374)", "60 cu in (D-13374)", "60 cu in (D-13382)"],
    holdDownAssys: ["SD", "SD_MOTORIZED", "MD", "HD_Single", "HD_Dual", "XD", "XXD"]
  },
  "CPR-200": {
    widths: ["24", "30", "36", "42", "48", "54", "60", "66", "72"],
    backplateDiameters: ["27", "72"],
    hydThreadingDrives: ["22 cu in (D-12689)", "38 cu in (D-13374)", "60 cu in (D-13374)", "60 cu in (D-13382)"],
    holdDownAssys: ["SD", "SD_MOTORIZED", "MD", "HD_Single", "HD_Dual", "XD", "XXD"]
  },
  "CPR-300": {
    widths: ["30", "36", "42", "48", "54", "60", "66", "72"],
    backplateDiameters: ["27", "72"],
    hydThreadingDrives: ["22 cu in (D-12689)", "38 cu in (D-13374)", "60 cu in (D-13374)", "60 cu in (D-13382)"],
    holdDownAssys: ["SD", "SD_MOTORIZED", "MD", "HD_Single", "HD_Dual", "XD", "XXD"]
  },
  "CPR-400": {
    widths: ["36", "42", "48", "54", "60", "66", "72"],
    backplateDiameters: ["27", "72"],
    hydThreadingDrives: ["22 cu in (D-12689)", "38 cu in (D-13374)", "60 cu in (D-13374)", "60 cu in (D-13382)"],
    holdDownAssys: ["SD", "SD_MOTORIZED", "MD", "HD_Single", "HD_Dual", "XD", "XXD"]
  },
  "CPR-500": {
    widths: ["42", "48", "54", "60", "66", "72", "78"],
    backplateDiameters: ["27", "72"],
    hydThreadingDrives: ["22 cu in (D-12689)", "38 cu in (D-13374)", "60 cu in (D-13374)", "60 cu in (D-13382)"],
    holdDownAssys: ["SD", "SD_MOTORIZED", "MD", "HD_Single", "HD_Dual", "XD", "XXD"]
  },
  "CPR-600": {
    widths: ["48", "54", "60", "66", "72", "78"],
    backplateDiameters: ["27", "72"],
    hydThreadingDrives: ["22 cu in (D-12689)", "38 cu in (D-13374)", "60 cu in (D-13374)", "60 cu in (D-13382)"],
    holdDownAssys: ["SD", "SD_MOTORIZED", "MD", "HD_Single", "HD_Dual", "XD", "XXD"]
  }
};

// Add hold down assembly to cylinder options mapping (model-specific)
export const HOLD_DOWN_CYLINDER_DEPENDENCIES: Record<string, Record<string, string[]>> = {
  "CPR-040": {
    "None": ["Air"],
    "LD_STANDARD": ["4in Air", "5in Air"],
    "LD_NARROW": ["4in Air"]
  },
  "CPR-060": {
    "None": ["Air"],
    "LD_STANDARD": ["4in Air", "5in Air", "8in Air"],
    "LD_NARROW": ["4in Air"],
    "LD_MOTORIZED": ["4in Air", "5in Air", "Hydraulic"]
  },
  "CPR-080": {
    "None": ["Air"],
    "LD_STANDARD": ["4in Air", "5in Air", "8in Air"],
    "LD_NARROW": ["4in Air"],
    "LD_MOTORIZED": ["4in Air", "5in Air", "Hydraulic"]
  },
  "CPR-100": {
    "SD": ["Air", "Hydraulic"],
    "SD_MOTORIZED": ["Air", "Hydraulic"],
    "MD": ["Hydraulic"],
    "HD_Single": ["Hydraulic"],
    "HD_Dual": ["Hydraulic"],
    "XD": ["Hydraulic"],
    "XXD": ["Hydraulic"]
  },
  "CPR-150": {
    "SD": ["Air", "Hydraulic"],
    "SD_MOTORIZED": ["Air", "Hydraulic"],
    "MD": ["Hydraulic"],
    "HD_Single": ["Hydraulic"],
    "HD_Dual": ["Hydraulic"],
    "XD": ["Hydraulic"],
    "XXD": ["Hydraulic"]
  },
  "CPR-200": {
    "SD": ["Air", "Hydraulic"],
    "SD_MOTORIZED": ["Air", "Hydraulic"],
    "MD": ["Hydraulic"],
    "HD_Single": ["Hydraulic"],
    "HD_Dual": ["Hydraulic"],
    "XD": ["Hydraulic"],
    "XXD": ["Hydraulic"]
  },
  "CPR-300": {
    "SD": ["Air", "Hydraulic"],
    "SD_MOTORIZED": ["Air", "Hydraulic"],
    "MD": ["Hydraulic"],
    "HD_Single": ["Hydraulic"],
    "HD_Dual": ["Hydraulic"],
    "XD": ["Hydraulic"],
    "XXD": ["Hydraulic"]
  },
  "CPR-400": {
    "SD": ["Air", "Hydraulic"],
    "SD_MOTORIZED": ["Air", "Hydraulic"],
    "MD": ["Hydraulic"],
    "HD_Single": ["Hydraulic"],
    "HD_Dual": ["Hydraulic"],
    "XD": ["Hydraulic"],
    "XXD": ["Hydraulic"]
  },
  "CPR-500": {
    "SD": ["Air", "Hydraulic"],
    "SD_MOTORIZED": ["Air", "Hydraulic"],
    "MD": ["Hydraulic"],
    "HD_Single": ["Hydraulic"],
    "HD_Dual": ["Hydraulic"],
    "XD": ["Hydraulic"],
    "XXD": ["Hydraulic"]
  },
  "CPR-600": {
    "SD": ["Air", "Hydraulic"],
    "SD_MOTORIZED": ["Air", "Hydraulic"],
    "MD": ["Hydraulic"],
    "HD_Single": ["Hydraulic"],
    "HD_Dual": ["Hydraulic"],
    "XD": ["Hydraulic"],
    "XXD": ["Hydraulic"]
  }
};

// Helper functions to get dependent options
export const getReelWidthOptionsForModel = (model: string) => {
  const dependencies = REEL_MODEL_DEPENDENCIES[model as keyof typeof REEL_MODEL_DEPENDENCIES];
  if (!dependencies) return REEL_WIDTH_OPTIONS;

  return dependencies.widths.map(width => ({ value: width, label: width }));
};

export const getBackplateDiameterOptionsForModel = (model: string) => {
  const dependencies = REEL_MODEL_DEPENDENCIES[model as keyof typeof REEL_MODEL_DEPENDENCIES];
  if (!dependencies) return BACKPLATE_DIAMETER_OPTIONS;

  return dependencies.backplateDiameters.map(diameter => ({ value: diameter, label: diameter }));
};

export const getHydThreadingDriveOptionsForModel = (model: string) => {
  const dependencies = REEL_MODEL_DEPENDENCIES[model as keyof typeof REEL_MODEL_DEPENDENCIES];
  if (!dependencies) return HYDRAULIC_THREADING_DRIVE_OPTIONS;

  return dependencies.hydThreadingDrives.map(drive => ({ value: drive, label: drive }));
};

export const getHoldDownAssyOptionsForModel = (model: string) => {
  const dependencies = REEL_MODEL_DEPENDENCIES[model as keyof typeof REEL_MODEL_DEPENDENCIES];
  if (!dependencies) return HOLD_DOWN_ASSY_OPTIONS;

  return dependencies.holdDownAssys.map(assy => ({ value: assy, label: HOLD_DOWN_ASSY_OPTIONS.find(opt => opt.value === assy)?.label || assy }));
};

export const getCylinderOptionsForHoldDownAssy = (model: string, holdDownAssy: string) => {
  const modelDependencies = HOLD_DOWN_CYLINDER_DEPENDENCIES[model];
  if (!modelDependencies) return CYLINDER_OPTIONS;

  const cylinderOptions = modelDependencies[holdDownAssy];
  if (!cylinderOptions) return CYLINDER_OPTIONS;

  return cylinderOptions.map(cylinder => ({ value: cylinder, label: cylinder }));
};

// STR model dependency mappings
export const STR_MODEL_DEPENDENCIES = {
  "CPPS-250": {
    widths: ["6", "12", "18", "24", "30", "36", "42", "48", "54"],
    horsepowers: ["3", "5", "7.5"],
    feedRatesByHorsepower: {
      "3": ["42", "65", "85", "105", "165"],
      "5": ["42", "65", "85", "105", "165"],
      "7.5": ["42", "65", "85", "105", "165"]
    }
  },
  "CPPS-306": {
    widths: ["24", "30", "36", "42", "48", "54", "60", "66", "72"],
    horsepowers: ["20", "25", "30", "40", "50"],
    feedRatesByHorsepower: {
      "20": ["80", "100", "120", "140", "160", "200"],
      "25": ["80", "100", "120", "140", "160", "200"],
      "30": ["80", "100", "120", "140", "160", "200"],
      "40": ["80", "100", "120", "140", "160", "200"],
      "50": ["80", "100", "120", "140", "160", "200"]
    }
  },
  "CPPS-350": {
    widths: ["12", "18", "24", "30", "36", "42", "48", "54", "60"],
    horsepowers: ["5", "7.5", "10", "15", "20"],
    feedRatesByHorsepower: {
      "5": ["57", "76", "92", "115", "153"],
      "7.5": ["65", "72", "96", "115", "153"],
      "10": ["65", "79", "95", "115", "153"],
      "15": ["69", "84", "95", "122", "157", "177"],
      "20": ["73", "101", "116", "146", "224"]
    }
  },
  "CPPS-406": {
    widths: ["24", "30", "36", "42", "48", "54", "60", "66", "72", "78"],
    horsepowers: ["20", "25", "30", "40", "50", "60", "75"],
    feedRatesByHorsepower: {
      "20": ["80", "100", "120", "140", "160", "180", "200"],
      "25": ["80", "100", "120", "140", "160", "180", "200"],
      "30": ["80", "100", "120", "140", "160", "180", "200"],
      "40": ["80", "100", "120", "140", "160", "180", "200"],
      "50": ["80", "100", "120", "140", "160", "180", "200"],
      "60": ["80", "100", "120", "140", "160", "180", "200"],
      "75": ["100", "120", "140", "160", "200"]
    }
  },
  "CPPS-507": {
    widths: ["24", "30", "36", "42", "48", "54", "60", "66", "72", "78"],
    horsepowers: ["40", "50", "60", "75", "100", "125"],
    feedRatesByHorsepower: {
      "40": ["80", "100", "120", "140", "160", "200"],
      "50": ["80", "100", "120", "140", "160", "200"],
      "60": ["80", "100", "120", "140", "160", "200"],
      "75": ["80", "100", "120", "140", "160", "200"],
      "100": ["120", "140", "160", "200"],
      "125": ["120", "140", "160", "200"]
    }
  },
  "SPGPS-810": {
    widths: ["24", "30", "36", "42", "48", "54", "60", "66", "72", "78"],
    horsepowers: ["40", "50", "60", "75", "100", "125"],
    feedRatesByHorsepower: {
      "40": ["80", "100", "120", "140", "160", "200"],
      "50": ["80", "100", "120", "140", "160", "200"],
      "60": ["80", "100", "120", "140", "160", "200"],
      "75": ["80", "100", "120", "140", "160", "200"],
      "100": ["120", "140", "160", "200"],
      "125": ["120", "140", "160", "200"]
    }
  }
};

// Helper functions to get STR dependent options
export const getStrWidthOptionsForModel = (model: string) => {
  const dependencies = STR_MODEL_DEPENDENCIES[model as keyof typeof STR_MODEL_DEPENDENCIES];
  if (!dependencies) return STR_WIDTH_OPTIONS;

  return dependencies.widths.map(width => ({ value: width, label: `${width}"` }));
};

export const getStrHorsepowerOptionsForModel = (model: string) => {
  const dependencies = STR_MODEL_DEPENDENCIES[model as keyof typeof STR_MODEL_DEPENDENCIES];
  if (!dependencies) return STR_HORSEPOWER_OPTIONS;

  return dependencies.horsepowers.map(hp => ({ value: hp, label: `${hp} HP` }));
};

export const getStrFeedRateOptionsForModelAndHorsepower = (model: string, horsepower: string) => {
  const dependencies = STR_MODEL_DEPENDENCIES[model as keyof typeof STR_MODEL_DEPENDENCIES];
  if (!dependencies) return STR_FEED_RATE_OPTIONS;

  // Find the feed rates for the specific model and horsepower
  switch (model) {
    case "CPPS-250":
      if (["3", "5", "7.5"].includes(horsepower)) {
        return ["42", "65", "85", "105", "165"].map(rate => ({ value: rate, label: `${rate} ft/min` }));
      }
      break;
    case "CPPS-306":
      if (["20", "25", "30", "40", "50"].includes(horsepower)) {
        return ["80", "100", "120", "140", "160", "200"].map(rate => ({ value: rate, label: `${rate} ft/min` }));
      }
      break;
    case "CPPS-350":
      if (horsepower === "5") return ["57", "76", "92", "115", "153"].map(rate => ({ value: rate, label: `${rate} ft/min` }));
      if (horsepower === "7.5") return ["65", "72", "96", "115", "153"].map(rate => ({ value: rate, label: `${rate} ft/min` }));
      if (horsepower === "10") return ["65", "79", "95", "115", "153"].map(rate => ({ value: rate, label: `${rate} ft/min` }));
      if (horsepower === "15") return ["69", "84", "95", "122", "157", "177"].map(rate => ({ value: rate, label: `${rate} ft/min` }));
      if (horsepower === "20") return ["73", "101", "116", "146", "224"].map(rate => ({ value: rate, label: `${rate} ft/min` }));
      break;
    case "CPPS-406":
      if (["20", "25", "30", "40", "50", "60"].includes(horsepower)) {
        return ["80", "100", "120", "140", "160", "180", "200"].map(rate => ({ value: rate, label: `${rate} ft/min` }));
      }
      if (horsepower === "75") {
        return ["100", "120", "140", "160", "200"].map(rate => ({ value: rate, label: `${rate} ft/min` }));
      }
      break;
    case "CPPS-507":
    case "SPGPS-810":
      if (["40", "50", "60", "75"].includes(horsepower)) {
        return ["80", "100", "120", "140", "160", "200"].map(rate => ({ value: rate, label: `${rate} ft/min` }));
      }
      if (["100", "125"].includes(horsepower)) {
        return ["120", "140", "160", "200"].map(rate => ({ value: rate, label: `${rate} ft/min` }));
      }
      break;
  }

  return STR_FEED_RATE_OPTIONS;
};

// Feed model dependency mappings
export const FEED_MODEL_DEPENDENCIES = {
  "CPRF-S1": {
    widths: ["6", "12", "18", "24"]
  },
  "CPRF-S2": {
    widths: ["6", "12", "18", "24", "30"]
  },
  "CPRF-S2 PLUS": {
    widths: ["6", "12", "18", "24", "30"]
  },
  "CPRF-S3": {
    widths: ["12", "18", "24", "30", "36", "42"]
  },
  "CPRF-S3 PLUS": {
    widths: ["12", "18", "24", "30", "36", "42"]
  },
  "CPRF-S4": {
    widths: ["12", "18", "24", "30", "36", "42", "48", "54"]
  },
  "CPRF-S4 PLUS": {
    widths: ["12", "18", "24", "30", "36", "42", "48", "54"]
  },
  "CPRF-S5": {
    widths: ["18", "24", "30", "36", "42", "48", "54", "60"]
  },
  "CPRF-S6": {
    widths: ["18", "24", "30", "36", "42", "48", "54", "60", "66", "72"]
  },
  "CPRF-S7": {
    widths: ["24", "30", "36", "42", "48", "54", "60", "66", "72"]
  },
  "CPRF-S8": {
    widths: ["24", "30", "36", "42", "48", "54", "60", "66", "72", "78"]
  }
};

// Helper function to get feed machine width options for model
export const getFeedMachineWidthOptionsForModel = (model: string) => {
  const dependencies = FEED_MODEL_DEPENDENCIES[model as keyof typeof FEED_MODEL_DEPENDENCIES];
  if (!dependencies) return MACHINE_WIDTH_OPTIONS;

  return dependencies.widths.map(width => ({ value: width, label: `${width}"` }));
};

// | "rfq"
// | "material-specs"
// | "tddbhd"
// | "reel-drive"
// | "str-utility"
// | "roll-str-backbend"
// | "feed"
// | "shear"
// | "summary-report";

export const RFQ_PERFORMANCE_SHEET_SEED = {
  id: "tab-rfq",
  label: "RFQ",
  value: "rfq",
  sequence: 1,
  sections: [
    {
      id: "section-basic-info",
      title: "Basic Information",
      sequence: 1,
      columns: 4,
      fields: [
        {
          id: "referenceNumber",
          label: "Reference Number",
          type: "text",
          size: 1,
          sequence: 1,
          required: true,
        },
        {
          id: "rfq.dates.date",
          label: "RFQ Date",
          type: "date",
          size: 1,
          sequence: 2,
          required: true,
          default: new Date().toISOString().split("T")[0],
        },
        {
          id: "common.customer",
          label: "Customer",
          type: "text",
          size: 1,
          sequence: 3,
          required: true,
        },
        {
          id: "common.customerInfo.contactName",
          label: "Contact Name",
          type: "text",
          size: 1,
          sequence: 4,
          required: true,
        },
        {
          id: "common.customerInfo.position",
          label: "Position",
          type: "text",
          size: 1,
          sequence: 5,
          required: true,
        },
        {
          id: "common.customerInfo.phoneNumber",
          label: "Phone Number",
          type: "text",
          size: 1,
          sequence: 6,
          required: true,
        },
        {
          id: "common.customerInfo.email",
          label: "Email",
          type: "text",
          size: 1,
          sequence: 7,
          required: true,
        },
        {
          id: "common.customerInfo.streetAddress",
          label: "Street Address",
          type: "text",
          size: 2,
          sequence: 8,
          required: true,
        },
        {
          id: "common.customerInfo.city",
          label: "City",
          type: "text",
          size: 1,
          sequence: 9,
          required: true,
        },
        {
          id: "common.customerInfo.state",
          label: "State",
          type: "text",
          size: 1,
          sequence: 10,
          required: true,
        },
        {
          id: "common.customerInfo.zip",
          label: "ZIP Code",
          type: "text",
          size: 1,
          sequence: 11,
          required: true,
        },
        {
          id: "common.customerInfo.country",
          label: "Country",
          type: "text",
          size: 1,
          sequence: 12,
          required: true,
        },
        {
          id: "common.customerInfo.dealerName",
          label: "Dealer Name",
          type: "text",
          size: 1,
          sequence: 13,
          required: true,
        },
        {
          id: "common.customerInfo.dealerSalesman",
          label: "Dealer Salesman",
          type: "text",
          size: 1,
          sequence: 14,
          required: true,
        },
        {
          id: "common.customerInfo.daysPerWeek",
          label: "Days per Week Operating",
          type: "select",
          size: 1,
          sequence: 15,
          required: false,
          options: DAYS_PER_WEEK_OPTIONS,
        },
        {
          id: "common.customerInfo.shiftsPerDay",
          label: "Shifts per Day",
          type: "select",
          size: 1,
          sequence: 16,
          required: false,
          options: DAYS_PER_WEEK_OPTIONS,
        },
        {
          id: "rfq.dates.idealDeliveryDate",
          label: "Ideal Delivery Date",
          type: "date",
          size: 1,
          sequence: 17,
          required: true,
        },
        {
          id: "rfq.dates.decisionDate",
          label: "Decision Date",
          type: "date",
          size: 1,
          sequence: 18,
          required: true,
        },
        {
          id: "rfq.dates.earliestDeliveryDate",
          label: "Earliest Delivery Date",
          type: "date",
          size: 1,
          sequence: 19,
          required: false,
        },
        {
          id: "rfq.dates.latestDeliveryDate",
          label: "Latest Delivery Date",
          type: "date",
          size: 1,
          sequence: 20,
          required: false,
        },
      ],
    },
    {
      id: "section-line-config",
      title: "Line Configuration",
      sequence: 2,
      columns: 4,
      fields: [
        {
          id: "feed.feed.application",
          label: "Application",
          type: "select",
          size: 2,
          sequence: 1,
          required: true,
          options: LINE_APPLICATION_OPTIONS,
          default: "Press Feed",
        },
        {
          id: "common.equipment.feed.lineType",
          label: "Line Type",
          type: "select",
          size: 1,
          sequence: 2,
          required: true,
          options: LINE_TYPE_OPTIONS,
          default: "Conventional",
        },
        {
          id: "feed.feed.pullThru.isPullThru",
          label: "Pull Through",
          type: "select",
          size: 1,
          sequence: 3,
          required: true,
          options: PULL_THRU_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.runningCosmeticMaterial",
          label: "Running Cosmetic Material",
          type: "select",
          size: 1,
          sequence: 4,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
      ],
    },
    {
      id: "section-coil-specs",
      title: "Coil Specifications",
      sequence: 3,
      columns: 4,
      fields: [
        {
          id: "common.coil.maxCoilWidth",
          label: "Max Coil Width (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: true,
          validation: {
            min: "common.coil.minCoilWidth",
          },
        },
        {
          id: "common.coil.minCoilWidth",
          label: "Min Coil Width (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: true,
          validation: {
            max: "common.coil.maxCoilWidth",
          },
        },
        {
          id: "common.coil.maxCoilOD",
          label: "Max Coil OD (in)",
          type: "number",
          size: 1,
          sequence: 3,
          required: true,
        },
        {
          id: "common.coil.coilID",
          label: "Coil ID (in)",
          type: "number",
          size: 1,
          sequence: 4,
          required: true,
        },
        {
          id: "common.coil.maxCoilWeight",
          label: "Max Coil Weight (lbs)",
          type: "number",
          size: 1,
          sequence: 5,
          required: true,
          validation: {
            max: "common.coil.maxCoilHandlingCap",
          },
        },
        {
          id: "common.coil.maxCoilHandlingCap",
          label: "Max Coil Handling Capacity (lbs)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
          validation: {
            min: "common.coil.maxCoilWeight",
          },
        },
        {
          id: "rfq.coil.slitEdge",
          label: "Slit Edge",
          type: "select",
          size: 1,
          sequence: 7,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.coil.millEdge",
          label: "Mill Edge",
          type: "select",
          size: 1,
          sequence: 8,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.coil.requireCoilCar",
          label: "Require Coil Car",
          type: "select",
          size: 1,
          sequence: 9,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.coil.runningOffBackplate",
          label: "Running Off Backplate",
          type: "select",
          size: 1,
          sequence: 10,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.coil.requireRewinding",
          label: "Require Rewinding",
          type: "select",
          size: 1,
          sequence: 11,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.coil.changeTimeConcern",
          label: "Change Time Concern",
          type: "select",
          size: 1,
          sequence: 12,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.coil.timeChangeGoal",
          label: "Time Change Goal (mins)",
          type: "number",
          size: 1,
          sequence: 13,
          required: false,
        },
        {
          id: "rfq.coil.loading",
          label: "Coil Loading Method",
          type: "select",
          size: 1,
          sequence: 14,
          required: true,
          options: COIL_LOADING_OPTIONS,
          default: "Operator Side",
        },
        {
          id: "common.equipment.feed.direction",
          label: "Feed Direction",
          type: "select",
          size: 1,
          sequence: 15,
          required: true,
          options: FEED_DIRECTION_OPTIONS,
          default: "left-to-right",
        },
      ],
    },
    {
      id: "section-material-scenario-1",
      title: "Material Specification - Scenario 1 (Primary)",
      sequence: 4,
      columns: 4,
      fields: [
        {
          id: "common.materialScenarios[0].materialThickness",
          label: "Material Thickness (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: true,
        },
        {
          id: "common.materialScenarios[0].materialWidth",
          label: "Material Width (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: true,
        },
        {
          id: "common.materialScenarios[0].materialType",
          label: "Material Type",
          type: "select",
          size: 1,
          sequence: 3,
          required: true,
          options: MATERIAL_TYPE_OPTIONS,
        },
        {
          id: "common.materialScenarios[0].maxYieldStrength",
          label: "Max Yield Strength (PSI)",
          type: "number",
          size: 1,
          sequence: 4,
          required: true,
        },
        {
          id: "common.materialScenarios[0].maxTensileStrength",
          label: "Max Tensile Strength (PSI)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
      ],
    },
    {
      id: "section-material-scenario-2",
      title: "Material Specification - Scenario 2",
      sequence: 5,
      columns: 4,
      fields: [
        {
          id: "common.materialScenarios[1].materialThickness",
          label: "Material Thickness (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "common.materialScenarios[1].materialWidth",
          label: "Material Width (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "common.materialScenarios[1].materialType",
          label: "Material Type",
          type: "select",
          size: 1,
          sequence: 3,
          required: false,
          options: MATERIAL_TYPE_OPTIONS,
        },
        {
          id: "common.materialScenarios[1].maxYieldStrength",
          label: "Max Yield Strength (PSI)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "common.materialScenarios[1].maxTensileStrength",
          label: "Max Tensile Strength (PSI)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
      ],
    },
    {
      id: "section-material-scenario-3",
      title: "Material Specification - Scenario 3",
      sequence: 6,
      columns: 4,
      fields: [
        {
          id: "common.materialScenarios[2].materialThickness",
          label: "Material Thickness (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "common.materialScenarios[2].materialWidth",
          label: "Material Width (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "common.materialScenarios[2].materialType",
          label: "Material Type",
          type: "select",
          size: 1,
          sequence: 3,
          required: false,
          options: MATERIAL_TYPE_OPTIONS,
        },
        {
          id: "common.materialScenarios[2].maxYieldStrength",
          label: "Max Yield Strength (PSI)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "common.materialScenarios[2].maxTensileStrength",
          label: "Max Tensile Strength (PSI)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
      ],
    },
    {
      id: "section-material-scenario-4",
      title: "Material Specification - Scenario 4",
      sequence: 7,
      columns: 4,
      fields: [
        {
          id: "common.materialScenarios[3].materialThickness",
          label: "Material Thickness (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "common.materialScenarios[3].materialWidth",
          label: "Material Width (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "common.materialScenarios[3].materialType",
          label: "Material Type",
          type: "select",
          size: 1,
          sequence: 3,
          required: false,
          options: MATERIAL_TYPE_OPTIONS,
        },
        {
          id: "common.materialScenarios[3].maxYieldStrength",
          label: "Max Yield Strength (PSI)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "common.materialScenarios[3].maxTensileStrength",
          label: "Max Tensile Strength (PSI)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
      ],
    },
    {
      id: "section-press-info",
      title: "Press Information",
      sequence: 8,
      columns: 4,
      fields: [
        {
          id: "rfq.press.gapFramePress",
          label: "Gap Frame Press",
          type: "checkbox",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rfq.press.hydraulicPress",
          label: "Hydraulic Press",
          type: "checkbox",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "rfq.press.obi",
          label: "OBI",
          type: "checkbox",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "rfq.press.servoPress",
          label: "Servo Press",
          type: "checkbox",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "rfq.press.shearDieApplication",
          label: "Shear Die Application",
          type: "checkbox",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "rfq.press.straightSidePress",
          label: "Straight Side Press",
          type: "checkbox",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "rfq.press.other",
          label: "Other",
          type: "checkbox",
          size: 1,
          sequence: 7,
          required: false,
        },
        {
          id: "rfq.press.tonnageOfPress",
          label: "Tonnage of Press",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
        },
        {
          id: "rfq.press.strokeLength",
          label: "Stroke Length (in)",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
        },
        {
          id: "rfq.press.maxSPM",
          label: "Max SPM",
          type: "number",
          size: 1,
          sequence: 10,
          required: true,
        },
        {
          id: "rfq.press.bedWidth",
          label: "Bed Width (in)",
          type: "number",
          size: 1,
          sequence: 11,
          required: false,
        },
        {
          id: "common.press.bedLength",
          label: "Bed Length (in)",
          type: "number",
          size: 1,
          sequence: 12,
          required: false,
        },
        {
          id: "rfq.press.windowSize",
          label: "Window Size (in)",
          type: "number",
          size: 1,
          sequence: 13,
          required: false,
        },
      ],
    },
    {
      id: "section-dies-info",
      title: "Dies Information",
      sequence: 9,
      columns: 4,
      fields: [
        {
          id: "rfq.dies.transferDies",
          label: "Transfer Dies",
          type: "select",
          size: 1,
          sequence: 1,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.dies.progressiveDies",
          label: "Progressive Dies",
          type: "select",
          size: 1,
          sequence: 2,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.dies.blankingDies",
          label: "Blanking Dies",
          type: "select",
          size: 1,
          sequence: 3,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
      ],
    },
    {
      id: "section-feed-requirements",
      title: "Feed Requirements",
      sequence: 10,
      columns: 3,
      fields: [
        {
          id: "common.feedRates.average.length",
          label: "Average Length (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: true,
        },
        {
          id: "common.feedRates.average.spm",
          label: "Average SPM",
          type: "number",
          size: 1,
          sequence: 2,
          required: true,
        },
        {
          id: "common.feedRates.average.fpm",
          label: "Average FPM",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "common.feedRates.max.length",
          label: "Max Length (in)",
          type: "number",
          size: 1,
          sequence: 4,
          required: true,
        },
        {
          id: "common.feedRates.max.spm",
          label: "Max SPM",
          type: "number",
          size: 1,
          sequence: 5,
          required: true,
        },
        {
          id: "common.feedRates.max.fpm",
          label: "Max FPM",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
          readOnly: true,
        },
        {
          id: "common.feedRates.min.length",
          label: "Min Length (in)",
          type: "number",
          size: 1,
          sequence: 7,
          required: true,
        },
        {
          id: "common.feedRates.min.spm",
          label: "Min SPM",
          type: "number",
          size: 1,
          sequence: 8,
          required: true,
        },
        {
          id: "common.feedRates.min.fpm",
          label: "Min FPM",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
          readOnly: true,
        },
        {
          id: "rfq.press.feedWindowDegrees",
          label: "Feed Window (Degrees)",
          type: "number",
          size: 1,
          sequence: 10,
          required: false,
        },
        {
          id: "rfq.press.cycleTime",
          label: "Cycle Time (sec)",
          type: "number",
          size: 1,
          sequence: 11,
          required: false,
        },
      ],
    },
    {
      id: "section-space-mounting",
      title: "Space & Mounting Requirements",
      sequence: 11,
      columns: 4,
      fields: [
        {
          id: "rfq.equipmentSpaceLength",
          label: "Equipment Space Length (ft)",
          type: "number",
          size: 1,
          sequence: 1,
          required: true,
        },
        {
          id: "rfq.equipmentSpaceWidth",
          label: "Equipment Space Width (ft)",
          type: "number",
          size: 1,
          sequence: 2,
          required: true,
        },
        {
          id: "rfq.obstructions",
          label: "Obstructions",
          type: "textarea",
          size: 4,
          sequence: 3,
          required: true,
        },
        {
          id: "rfq.voltageRequired",
          label: "Voltage Required",
          type: "text",
          size: 1,
          sequence: 4,
          required: true,
        },
        {
          id: "rfq.mount.feederMountedToPress",
          label: "Feeder Mounted to Press",
          type: "select",
          size: 1,
          sequence: 5,
          required: false,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.mount.adequateSupport",
          label: "Adequate Support",
          type: "select",
          size: 1,
          sequence: 6,
          required: false,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.mount.customMounting",
          label: "Custom Mounting",
          type: "select",
          size: 1,
          sequence: 7,
          required: false,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "common.equipment.feed.passline",
          label: "Passline (in)",
          type: "select",
          size: 1,
          sequence: 8,
          required: true,
          options: PASSLINE_OPTIONS,
        },
        {
          id: "common.equipment.feed.loopPit",
          label: "Loop Pit",
          type: "select",
          size: 1,
          sequence: 9,
          required: false,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "rfq.requireGuarding",
          label: "Require Guarding",
          type: "select",
          size: 1,
          sequence: 10,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
      ],
    },
    {
      id: "section-special-requirements",
      title: "Special Requirements",
      sequence: 12,
      columns: 4,
      fields: [
        {
          id: "rfq.specialRequirements",
          label: "Special Requirements & Notes",
          type: "textarea",
          size: 4,
          sequence: 1,
          required: false,
        },
      ],
    },
  ],
};

export const MATERIAL_SPECS_PERFORMANCE_SHEET_SEED = {
  id: "tab-material-specs",
  label: "Material Specs",
  value: "material-specs",
  sequence: 2,
  sections: [
    {
      id: "section-customer-date",
      title: "Customer & Date",
      sequence: 1,
      columns: 2,
      fields: [
        {
          id: "common.customer",
          label: "Customer",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rfq.dates.date",
          label: "Date",
          type: "date",
          size: 1,
          sequence: 2,
          required: false,
        },
      ],
    },
    {
      id: "section-material-specifications-scenario-1",
      title: "Material Specifications - Scenario 1 (Primary)",
      sequence: 2,
      columns: 4,
      fields: [
        {
          id: "common.materialScenarios[0].materialWidth",
          label: "Coil Width (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: true,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[0].coilWeight",
          label: "Coil Weight (lbs)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[0].materialThickness",
          label: "Material Thickness (in)",
          type: "number",
          size: 1,
          sequence: 3,
          required: true,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[0].materialType",
          label: "Material Type",
          type: "select",
          size: 1,
          sequence: 4,
          required: true,
          readOnly: true,
          options: MATERIAL_TYPE_OPTIONS,
        },
        {
          id: "materialSpecs.scenarios[0].yieldStrength",
          label: "Yield Strength (psi)",
          type: "number",
          size: 1,
          sequence: 5,
          required: true,
        },
        {
          id: "materialSpecs.scenarios[0].materialTensile",
          label: "Material Tensile (psi)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "common.materialScenarios[0].reqMaxFPM",
          label: "Required Max FPM",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[0].minBendRadius",
          label: "Min Bend Radius (in)",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[0].minLoopLength",
          label: "Min Loop Length (ft)",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[0].coilOD",
          label: "Coil O.D.",
          type: "number",
          size: 1,
          sequence: 10,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[0].coilID",
          label: "Coil I.D.",
          type: "number",
          size: 1,
          sequence: 11,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[0].coilODCalculated",
          label: "Coil O.D. Calculated",
          type: "number",
          size: 1,
          sequence: 12,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-material-specifications-scenario-2",
      title: "Material Specifications - Scenario 2",
      sequence: 3,
      columns: 4,
      fields: [
        {
          id: "common.materialScenarios[1].materialWidth",
          label: "Coil Width (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[1].coilWeight",
          label: "Coil Weight (lbs)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[1].materialThickness",
          label: "Material Thickness (in)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[1].materialType",
          label: "Material Type",
          type: "select",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
          options: MATERIAL_TYPE_OPTIONS,
        },
        {
          id: "materialSpecs.scenarios[1].yieldStrength",
          label: "Yield Strength (psi)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "materialSpecs.scenarios[1].materialTensile",
          label: "Material Tensile (psi)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "common.materialScenarios[1].reqMaxFPM",
          label: "Required Max FPM",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[1].minBendRadius",
          label: "Min Bend Radius (in)",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[1].minLoopLength",
          label: "Min Loop Length (ft)",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[1].coilOD",
          label: "Coil O.D.",
          type: "number",
          size: 1,
          sequence: 10,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[1].coilID",
          label: "Coil I.D.",
          type: "number",
          size: 1,
          sequence: 11,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[1].coilODCalculated",
          label: "Coil O.D. Calculated",
          type: "number",
          size: 1,
          sequence: 12,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-material-specifications-scenario-3",
      title: "Material Specifications - Scenario 3",
      sequence: 4,
      columns: 4,
      fields: [
        {
          id: "common.materialScenarios[2].materialWidth",
          label: "Coil Width (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[2].coilWeight",
          label: "Coil Weight (lbs)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[2].materialThickness",
          label: "Material Thickness (in)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[2].materialType",
          label: "Material Type",
          type: "select",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
          options: MATERIAL_TYPE_OPTIONS,
        },
        {
          id: "materialSpecs.scenarios[2].yieldStrength",
          label: "Yield Strength (psi)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "materialSpecs.scenarios[2].materialTensile",
          label: "Material Tensile (psi)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "common.materialScenarios[2].reqMaxFPM",
          label: "Required Max FPM",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[2].minBendRadius",
          label: "Min Bend Radius (in)",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[2].minLoopLength",
          label: "Min Loop Length (ft)",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[2].coilOD",
          label: "Coil O.D.",
          type: "number",
          size: 1,
          sequence: 10,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[2].coilID",
          label: "Coil I.D.",
          type: "number",
          size: 1,
          sequence: 11,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[2].coilODCalculated",
          label: "Coil O.D. Calculated",
          type: "number",
          size: 1,
          sequence: 12,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-material-specifications-scenario-4",
      title: "Material Specifications - Scenario 4",
      sequence: 5,
      columns: 4,
      fields: [
        {
          id: "common.materialScenarios[3].materialWidth",
          label: "Coil Width (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[3].coilWeight",
          label: "Coil Weight (lbs)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[3].materialThickness",
          label: "Material Thickness (in)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[3].materialType",
          label: "Material Type",
          type: "select",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
          options: MATERIAL_TYPE_OPTIONS,
        },
        {
          id: "materialSpecs.scenarios[3].yieldStrength",
          label: "Yield Strength (psi)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "materialSpecs.scenarios[3].materialTensile",
          label: "Material Tensile (psi)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "common.materialScenarios[3].reqMaxFPM",
          label: "Required Max FPM",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[3].minBendRadius",
          label: "Min Bend Radius (in)",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[3].minLoopLength",
          label: "Min Loop Length (ft)",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[3].coilOD",
          label: "Coil O.D.",
          type: "number",
          size: 1,
          sequence: 10,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[3].coilID",
          label: "Coil I.D.",
          type: "number",
          size: 1,
          sequence: 11,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[3].coilODCalculated",
          label: "Coil O.D. Calculated",
          type: "number",
          size: 1,
          sequence: 12,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-other-specifications",
      title: "Other Specifications",
      sequence: 6,
      columns: 2,
      fields: [
        {
          id: "common.equipment.feed.direction",
          label: "Select Feed Direction",
          type: "select",
          size: 1,
          sequence: 1,
          required: true,
          options: FEED_DIRECTION_OPTIONS,
        },
        {
          id: "common.equipment.feed.controlsLevel",
          label: "Select Controls Level",
          type: "select",
          size: 1,
          sequence: 2,
          required: true,
          options: CONTROLS_LEVEL_OPTIONS,
        },
        {
          id: "common.equipment.feed.typeOfLine",
          label: "Type of Line",
          type: "select",
          size: 1,
          sequence: 3,
          required: true,
          options: TYPE_OF_LINE_OPTIONS,
        },
        {
          id: "common.equipment.feed.controls",
          label: "Feed Controls",
          type: "select",
          size: 1,
          sequence: 4,
          required: true,
          options: FEED_TYPE_OPTIONS,
          defaultValue: "Sigma 5",
          readOnly: true,
        },
        {
          id: "common.equipment.feed.passline",
          label: "Passline",
          type: "select",
          size: 1,
          sequence: 5,
          required: true,
          options: PASSLINE_OPTIONS,
        },
        {
          id: "materialSpecs.straightener.rolls.typeOfRoll",
          label: "Type of Roll",
          type: "select",
          size: 1,
          sequence: 6,
          required: false,
          options: ROLL_TYPE_OPTIONS,
        },
        {
          id: "materialSpecs.reel.backplate.type",
          label: "Reel Backplate Type",
          type: "select",
          size: 1,
          sequence: 7,
          required: true,
          options: BACKPLATE_TYPE_OPTIONS,
        },
        {
          id: "materialSpecs.reel.style",
          label: "Reel Style",
          type: "select",
          size: 1,
          sequence: 8,
          required: true,
          options: REEL_STYLE_OPTIONS,
        },
        {
          id: "common.equipment.feed.lightGuageNonMarking",
          label: "Light Gauge Non-Marking",
          type: "checkbox",
          size: 1,
          sequence: 9,
          required: false,
        },
        {
          id: "common.equipment.feed.nonMarking",
          label: "Non-Marking",
          type: "checkbox",
          size: 1,
          sequence: 10,
          required: false,
        },
      ],
    },
  ],
};

export const TDDBHD_PERFORMANCE_SHEET_SEED = {
  id: "tab-tddbhd",
  label: "TDDBHD",
  value: "tddbhd",
  sequence: 3,
  sections: [
    {
      id: "section-customer-date",
      title: "Customer & Date",
      sequence: 1,
      columns: 4,
      fields: [
        {
          id: "common.customer",
          label: "Customer",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rfq.dates.date",
          label: "Date",
          type: "date",
          size: 1,
          sequence: 2,
          required: false,
        },
      ],
    },
    {
      id: "section-reel-material-specs",
      title: "Reel & Material Specs",
      sequence: 2,
      columns: 4,
      fields: [
        {
          id: "common.equipment.reel.model",
          label: "Reel Model",
          type: "select",
          size: 1,
          sequence: 1,
          required: false,
          options: REEL_MODEL_OPTIONS,
          default: "CPR-040",
        },
        {
          id: "common.equipment.reel.width",
          label: "Reel Width",
          type: "select",
          size: 1,
          sequence: 2,
          required: false,
          options: REEL_WIDTH_OPTIONS,
          dependsOn: "common.equipment.reel.model",
          dependencyType: "reelWidth",
        },
        {
          id: "common.equipment.reel.backplate.diameter",
          label: "Backplate Diameter",
          type: "select",
          size: 1,
          sequence: 3,
          required: false,
          options: BACKPLATE_DIAMETER_OPTIONS,
          dependsOn: "common.equipment.reel.model",
          dependencyType: "backplateDiameter",
        },
        {
          id: "common.materialScenarios[0].materialType",
          label: "Material Type",
          type: "select",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
          options: MATERIAL_TYPE_OPTIONS,
        },
        {
          id: "common.materialScenarios[0].materialWidth",
          label: "Material Width (in)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
          readOnly: true,
        },
        {
          id: "common.materialScenarios[0].materialThickness",
          label: "Material Thickness (in)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.scenarios[0].yieldStrength",
          label: "Material Yield Strength (psi)",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.airPressureAvailable",
          label: "Air Pressure Available (psi)",
          type: "number",
          size: 1,
          sequence: 8,
          required: true,
        },
        {
          id: "tddbhd.reel.requiredDecelRate",
          label: "Required Decel. Rate (ft/sec²)",
          type: "number",
          size: 1,
          sequence: 9,
          required: true,
        },
      ],
    },
    {
      id: "section-coil-brake-specs",
      title: "Coil, Brake & Other Specs",
      sequence: 3,
      columns: 4,
      fields: [
        {
          id: "tddbhd.coil.coilWeight",
          label: "Coil Weight (lbs)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.coil.coilOD",
          label: "Coil O.D. (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.dispReelMtr",
          label: "Disp. (Reel) Mtr.",
          type: "text",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.webTension.psi",
          label: "Web Tension (psi)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.webTension.lbs",
          label: "Web Tension (lbs)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.brakePadDiameter",
          label: "Brake Pad Diameter (in)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.cylinderBore",
          label: "Cylinder Bore (in)",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.coefficientOfFriction",
          label: "Coefficient of Friction",
          type: "number",
          size: 1,
          sequence: 8,
          required: true,
        },
      ],
    },
    {
      id: "section-threading-drive",
      title: "Threading Drive",
      sequence: 4,
      columns: 4,
      fields: [
        {
          id: "tddbhd.reel.threadingDrive.airClutch",
          label: "Air Clutch",
          type: "select",
          size: 1,
          sequence: 1,
          required: true,
          options: YES_NO_OPTIONS,
          default: "no",
        },
        {
          id: "tddbhd.reel.threadingDrive.hydThreadingDrive",
          label: "Hyd. Threading Drive",
          type: "select",
          size: 1,
          sequence: 2,
          required: true,
          options: HYDRAULIC_THREADING_DRIVE_OPTIONS,
          dependsOn: "common.equipment.reel.model",
          dependencyType: "hydThreadingDrive",
        },
        {
          id: "tddbhd.reel.torque.atMandrel",
          label: "Torque At Mandrel (in. lbs.)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.torque.rewindRequired",
          label: "Rewind Torque Req. (in. lbs.)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.checks.tddbhdCheck",
          label: "Overall Check",
          type: "text",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "tddbhd.reel.checks.minMaterialWidthCheck",
          label: "Min Material Width Check",
          type: "text",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "tddbhd.reel.checks.airPressureCheck",
          label: "Air Pressure Check",
          type: "text",
          size: 1,
          sequence: 7,
          required: false,
        },
        {
          id: "tddbhd.reel.checks.rewindTorqueCheck",
          label: "Rewind Torque Check",
          type: "text",
          size: 1,
          sequence: 8,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.checks.holdDownForceCheck",
          label: "Hold Down Force Check",
          type: "text",
          size: 1,
          sequence: 9,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.checks.brakePressCheck",
          label: "Brake Press Check",
          type: "text",
          size: 1,
          sequence: 10,
          required: false,
        },
        {
          id: "tddbhd.reel.checks.torqueRequiredCheck",
          label: "Torque Required Check",
          type: "text",
          size: 1,
          sequence: 11,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-hold-down",
      title: "Hold Down",
      sequence: 5,
      columns: 4,
      fields: [
        {
          id: "tddbhd.reel.holddown.assy",
          label: "Hold Down Assy",
          type: "select",
          size: 1,
          sequence: 1,
          required: true,
          options: HOLD_DOWN_ASSY_OPTIONS,
          dependsOn: "common.equipment.reel.model",
          dependencyType: "holdDownAssy",
        },
        {
          id: "tddbhd.reel.holddown.cylinderPressure",
          label: "Holddown Pressure (psi)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "tddbhd.reel.holddown.force.required",
          label: "Hold Down Force Required (lbs)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.holddown.force.available",
          label: "Hold Down Force Available (lbs)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.holddown.cylinder",
          label: "Cylinder",
          type: "select",
          size: 1,
          sequence: 5,
          required: true,
          options: CYLINDER_OPTIONS,
          dependsOn: "tddbhd.reel.holddown.assy",
          dependencyType: "cylinder",
        },
        {
          id: "tddbhd.reel.minMaterialWidth",
          label: "Min. Material Width (in)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
          readonly: true,
        },
        {
          id: "tddbhd.reel.confirmedMinWidth",
          label: "Confirmed Min Width OK",
          type: "checkbox",
          size: 1,
          sequence: 6,
          required: false,
        },
      ],
    },
    {
      id: "section-drag-brake",
      title: "Drag Brake",
      sequence: 6,
      columns: 4,
      fields: [
        {
          id: "tddbhd.reel.dragBrake.model",
          label: "Brake Model",
          type: "select",
          size: 1,
          sequence: 1,
          required: true,
          options: BRAKE_MODEL_OPTIONS,
        },
        {
          id: "tddbhd.reel.dragBrake.quantity",
          label: "Brake Quantity",
          type: "select",
          size: 1,
          sequence: 2,
          required: true,
          options: BRAKE_QUANTITY_OPTIONS,
        },
        {
          id: "tddbhd.reel.torque.required",
          label: "Torque Required (in. lbs.)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.dragBrake.psiAirRequired",
          label: "Failsafe - Single Stage (psi air req.)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.dragBrake.holdingForce",
          label: "Failsafe Holding Force (in. lbs.)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
          readOnly: true,
        },
      ],
    },
  ],
};

export const REEL_DRIVE_PERFORMANCE_SHEET_SEED = {
  id: "tab-reel-drive",
  label: "Reel Drive",
  value: "reel-drive",
  sequence: 4,
  sections: [
    {
      id: "section-customer-date",
      title: "Customer & Date",
      sequence: 1,
      columns: 2,
      fields: [
        {
          id: "common.customer",
          label: "Customer",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rfq.dates.date",
          label: "Date",
          type: "date",
          size: 1,
          sequence: 2,
          required: false,
        },
      ],
    },
    {
      id: "section-model-hp",
      title: "Model & HP",
      sequence: 2,
      columns: 2,
      fields: [
        {
          id: "common.equipment.reel.model",
          label: "Reel Model",
          type: "select",
          size: 1,
          sequence: 1,
          required: false,
          options: REEL_MODEL_OPTIONS,
        },
        {
          id: "common.equipment.reel.horsepower",
          label: "HP",
          type: "select",
          size: 1,
          sequence: 2,
          required: false,
          options: REEL_HORSEPOWER_OPTIONS,
        },
      ],
    },
    {
      id: "section-reel",
      title: "Reel",
      sequence: 3,
      columns: 5,
      fields: [
        {
          id: "reelDrive.reel.size",
          label: "SIZE",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.reel.width",
          label: "MAX WIDTH",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "reelDrive.reel.bearing.distance",
          label: "BRG. DIST.",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.bearing.diameter.front",
          label: "F. BRG. DIA.",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.bearing.diameter.rear",
          label: "R. BRG. DIA.",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-mandrel",
      title: "Mandrel",
      sequence: 4,
      columns: 7,
      fields: [
        {
          id: "reelDrive.reel.mandrel.diameter",
          label: "DIAMETER",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.mandrel.length",
          label: "LENGTH",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.mandrel.maxRPM",
          label: "MAX RPM",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.mandrel.RpmFull",
          label: "RPM FULL",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.mandrel.weight",
          label: "WEIGHT",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.mandrel.inertia",
          label: "INERTIA",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.mandrel.reflInertia",
          label: "REFL. INERT.",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-backplate",
      title: "Backplate",
      sequence: 5,
      columns: 5,
      fields: [
        {
          id: "common.equipment.reel.backplate.diameter",
          label: "DIAMETER",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "reelDrive.reel.backplate.thickness",
          label: "THICKNESS",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.backplate.weight",
          label: "WEIGHT",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.backplate.inertia",
          label: "INERTIA",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.backplate.reflInertia",
          label: "REFL. INERT.",
          type: "number",
          size: 1,
          sequence: 5,
          readOnly: true,
          required: false,
        },
      ],
    },
    {
      id: "section-coil",
      title: "Coil",
      sequence: 6,
      columns: 7,
      fields: [
        {
          id: "reelDrive.coil.density",
          label: "DENSITY",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "common.coil.maxCoilOD",
          label: "O.D.",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "common.coil.coilID",
          label: "I.D.",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "common.material.coilWidth",
          label: "WIDTH",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "common.coil.maxCoilWeight",
          label: "WEIGHT",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "reelDrive.coil.inertia",
          label: "INERTIA",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "reelDrive.coil.reflInertia",
          label: "REFL. INERT.",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
      ],
    },
    {
      id: "section-reducer",
      title: "Reducer",
      sequence: 7,
      columns: 5,
      fields: [
        {
          id: "reelDrive.reel.reducer.ratio",
          label: "RATIO",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "reelDrive.reel.reducer.driving",
          label: "DRIVING",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "reelDrive.reel.reducer.backdriving",
          label: "BACKDRIVING",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "reelDrive.reel.reducer.inertia",
          label: "INERTIA",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "reelDrive.reel.reducer.reflInertia",
          label: "REFL. INERT.",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
      ],
    },
    {
      id: "section-chain",
      title: "Chain",
      sequence: 8,
      columns: 6,
      fields: [
        {
          id: "reelDrive.reel.chain.ratio",
          label: "RATIO",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "reelDrive.reel.chain.sprktOD",
          label: "SPRK. O.D.",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "reelDrive.reel.chain.sprktThickness",
          label: "SPRK. THK.",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "reelDrive.reel.chain.weight",
          label: "WEIGHT",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "reelDrive.reel.chain.inertia",
          label: "INERTIA",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "reelDrive.reel.chain.reflInertia",
          label: "REFL. INERT.",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
      ],
    },
    {
      id: "section-total",
      title: "Total",
      sequence: 9,
      columns: 3,
      fields: [
        {
          id: "reelDrive.reel.ratio",
          label: "RATIO",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "reelDrive.reel.totalReflInertia.empty",
          label: "TOTAL REFL. INERTIA EMPTY",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "reelDrive.reel.totalReflInertia.full",
          label: "TOTAL REFL. INERTIA FULL",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
      ],
    },
    {
      id: "section-motor",
      title: "Motor",
      sequence: 10,
      columns: 4,
      fields: [
        {
          id: "common.equipment.reel.horsepower",
          label: "HP",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "reelDrive.reel.motor.inertia",
          label: "INERTIA",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "reelDrive.reel.motor.rpm.base",
          label: "BASE RPM",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "reelDrive.reel.motor.rpm.full",
          label: "RPM FULL",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
      ],
    },
    {
      id: "section-friction",
      title: "Friction",
      sequence: 11,
      columns: 4,
      fields: [
        {
          id: "reelDrive.reel.friction.bearing.mandrel.rear",
          label: "R. BRG. MAND.",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "reelDrive.reel.friction.bearing.mandrel.front",
          label: "F. BRG. MAND.",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "reelDrive.reel.friction.bearing.coil.front",
          label: "F. BRG. COIL",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "reelDrive.reel.friction.bearing.coil.rear",
          label: "R. BRG. COIL",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "reelDrive.reel.friction.bearing.total.empty",
          label: "TOTAL EMPTY",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "reelDrive.reel.friction.bearing.total.full",
          label: "TOTAL FULL",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "reelDrive.reel.friction.bearing.refl.empty",
          label: "REFL. EMPTY",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
        {
          id: "reelDrive.reel.friction.bearing.refl.full",
          label: "REFL. FULL",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
        },
      ],
    },
    {
      id: "section-speed-acceleration",
      title: "Speed & Acceleration",
      sequence: 12,
      columns: 3,
      fields: [
        {
          id: "reelDrive.reel.speed",
          label: "SPEED",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "reelDrive.reel.accelerationRate",
          label: "ACCEL RATE",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "reelDrive.reel.accelerationTime",
          label: "ACCEL TIME",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
      ],
    },
    {
      id: "section-torque",
      title: "Torque",
      sequence: 13,
      columns: 2,
      fields: [
        {
          id: "reelDrive.reel.torque.empty.torque",
          label: "EMPTY",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.torque.full.torque",
          label: "FULL",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-hp-required",
      title: "HP Required",
      sequence: 14,
      columns: 2,
      fields: [
        {
          id: "reelDrive.reel.torque.empty.horsepowerRequired",
          label: "EMPTY",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.torque.full.horsepowerRequired",
          label: "FULL",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.torque.empty.horsepowerCheck",
          label: "HP Check (Empty)",
          type: "text",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.torque.full.horsepowerCheck",
          label: "HP Check (Full)",
          type: "text",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-regen",
      title: "Regen",
      sequence: 15,
      columns: 2,
      fields: [
        {
          id: "reelDrive.reel.torque.empty.regen",
          label: "EMPTY",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.torque.full.regen",
          label: "FULL",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.torque.empty.regenCheck",
          label: "Regen Check (Empty)",
          type: "text",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.torque.full.regenCheck",
          label: "Regen Check (Full)",
          type: "text",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
      ],
    },
  ],
};

export const STR_UTILITY_PERFORMANCE_SHEET_SEED = {
  id: "tab-str-utility",
  label: "Str Utility",
  value: "str-utility",
  sequence: 5,
  sections: [
    {
      id: "section-customer-date",
      title: "Customer & Date",
      sequence: 1,
      columns: 2,
      fields: [
        {
          id: "common.customer",
          label: "Customer",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rfq.dates.date",
          label: "Date",
          type: "date",
          size: 1,
          sequence: 2,
          required: false,
        },
      ],
    },
    {
      id: "section-straightener-specifications",
      title: "Straightener Specifications",
      sequence: 2,
      columns: 4,
      fields: [
        {
          id: "strUtility.straightener.payoff",
          label: "Payoff",
          type: "select",
          size: 1,
          sequence: 1,
          required: false,
          options: PAYOFF_OPTIONS,
          default: "top",
        },
        {
          id: "common.equipment.straightener.model",
          label: "Str. Model",
          type: "select",
          size: 1,
          sequence: 2,
          required: false,
          options: STR_MODEL_OPTIONS,
        },
        {
          id: "common.equipment.straightener.width",
          label: "Str. Width (in.)",
          type: "select",
          size: 1,
          sequence: 3,
          required: false,
          options: STR_WIDTH_OPTIONS,
          dependsOn: "common.equipment.straightener.model",
          dependencyType: "strWidth",
        },
        {
          id: "common.equipment.straightener.numberOfRolls",
          label: "No. of Str. Rolls",
          type: "number",
          size: 1,
          sequence: 4,
          required: true,
        },
      ],
    },
    {
      id: "section-coil-information",
      title: "Coil Information",
      sequence: 3,
      columns: 3,
      fields: [
        {
          id: "strUtility.coil.maxCoilWeight",
          label: "Coil Wt. Capacity (lbs)",
          type: "number",
          size: 1,
          sequence: 1,
          required: true,
        },
        {
          id: "common.coil.coilID",
          label: "Coil ID. (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: true,
        },
        {
          id: "common.material.coilWidth",
          label: "Coil Width (in)",
          type: "number",
          size: 1,
          sequence: 3,
          required: true,
        },
        {
          id: "common.material.materialThickness",
          label: "Thickness (in)",
          type: "number",
          size: 1,
          sequence: 4,
          required: true,
        },
        {
          id: "common.material.yieldStrength",
          label: "Yield Strength (psi)",
          type: "number",
          size: 1,
          sequence: 5,
          required: true,
        },
        {
          id: "common.material.materialType",
          label: "Material",
          type: "select",
          size: 1,
          sequence: 6,
          required: false,
          options: MATERIAL_TYPE_OPTIONS,
        },
      ],
    },
    {
      id: "section-operating-parameters",
      title: "Operating Parameters",
      sequence: 4,
      columns: 4,
      fields: [
        {
          id: "strUtility.straightener.horsepower",
          label: "Horse Power (HP)",
          type: "select",
          size: 1,
          sequence: 1,
          required: false,
          options: STR_HORSEPOWER_OPTIONS,
          dependsOn: "common.equipment.straightener.model",
          dependencyType: "strHorsepower",
        },
        {
          id: "strUtility.straightener.acceleration",
          label: "Acceleration (ft/sec²)",
          type: "number",
          size: 1,
          sequence: 2,
          required: true,
        },
        {
          id: "strUtility.straightener.feedRate",
          label: "Feed Rate (ft/min)",
          type: "select",
          size: 1,
          sequence: 3,
          required: false,
          options: STR_FEED_RATE_OPTIONS,
          dependsOn: "strUtility.straightener.horsepower",
          dependencyType: "strFeedRate",
          secondaryDependsOn: "common.equipment.straightener.model",
        },
        {
          id: "strUtility.straightener.autoBrakeCompensation",
          label: "Auto. Brake Compen.",
          type: "select",
          size: 1,
          sequence: 4,
          required: false,
          options: YES_NO_OPTIONS,
        },
      ],
    },
    {
      id: "section-physical-parameters",
      title: "Physical Parameters",
      sequence: 5,
      columns: 3,
      fields: [
        {
          id: "strUtility.straightener.rolls.straightener.diameter",
          label: "Str. Roll Dia. (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "strUtility.straightener.rolls.pinch.diameter",
          label: "Pinch Roll Dia. (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "strUtility.straightener.centerDistance",
          label: "Center Dist. (in)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "strUtility.straightener.jackForceAvailable",
          label: "Jack Force Avail. (lbs)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "strUtility.straightener.maxRollDepth",
          label: "Max. Roll Depth (in)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
          readOnly: true,
        },
        {
          id: "strUtility.straightener.modulus",
          label: "Modulus (psi)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-gears-data",
      title: "Gears Data",
      sequence: 6,
      columns: 4,
      fields: [
        {
          id: "strUtility.straightener.gear.pinchRoll.numberOfTeeth",
          label: "Pinch Roll Gear - # teeth",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "strUtility.straightener.gear.pinchRoll.dp",
          label: "Pinch Roll Gear - DP",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "strUtility.straightener.gear.straightenerRoll.numberOfTeeth",
          label: "Str. Roll Gear - # teeth",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "strUtility.straightener.gear.straightenerRoll.dp",
          label: "Str. Roll Gear - DP",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "strUtility.straightener.gear.faceWidth",
          label: "Face Width (in)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "strUtility.straightener.gear.contAngle",
          label: "Cont. Angle (degree)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
      ],
    },
    {
      id: "section-calculations",
      title: "Force Calculations",
      sequence: 7,
      columns: 2,
      fields: [
        {
          id: "strUtility.straightener.required.force",
          label: "Required Force (lbs)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "strUtility.straightener.jackForceAvailable",
          label: "Rated Force (lbs)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "strUtility.straightener.rolls.pinch.requiredGearTorque",
          label: "Pinch Roll Req. Torque",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "strUtility.straightener.rolls.pinch.ratedTorque",
          label: "Pinch Roll Rated Torque",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "strUtility.straightener.rolls.straightener.requiredGearTorque",
          label: "Str. Roll Req. Torque",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "strUtility.straightener.rolls.straightener.ratedTorque",
          label: "Str. Roll Rated Torque",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "strUtility.straightener.required.horsepower",
          label: "Horse Power Required (HP)",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
      ],
    },
    {
      id: "section-additional-calculations",
      title: "Additional Calculations",
      sequence: 8,
      columns: 3,
      fields: [
        {
          id: "strUtility.straightener.actualCoilWeight",
          label: "Actual Coil Wt. (lbs)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "strUtility.straightener.coilOD",
          label: "Coil OD. (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "strUtility.straightener.torque.straightener",
          label: "Str. Torque (in lbs)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "strUtility.straightener.torque.acceleration",
          label: "Accel. Torque (in lbs)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "strUtility.straightener.torque.brake",
          label: "Brake Torque (in lbs)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
      ],
    },
    {
      id: "section-checks",
      title: "Validation Checks",
      sequence: 7,
      columns: 4,
      fields: [
        {
          id: "strUtility.straightener.required.horsepowerCheck",
          label: "Horsepower Check",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "strUtility.straightener.required.jackForceCheck",
          label: "Jack Force Check",
          type: "text",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "strUtility.straightener.required.backupRollsCheck",
          label: "Backup Rolls Check",
          type: "text",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "strUtility.straightener.required.feedRateCheck",
          label: "Feed Rate Check",
          type: "text",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "strUtility.straightener.required.pinchRollCheck",
          label: "Pinch Roll Check",
          type: "text",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "strUtility.straightener.required.strRollCheck",
          label: "Str Roll Check",
          type: "text",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "strUtility.straightener.required.fpmCheck",
          label: "FPM Check",
          type: "text",
          size: 1,
          sequence: 7,
          required: false,
        },
      ],
    },
  ],
};

export const ROLL_STR_BACKBEND_PERFORMANCE_SHEET_SEED = {
  id: "tab-roll-str-backbend",
  label: "Roll Str Backbend",
  value: "roll-str-backbend",
  sequence: 6,
  sections: [
    {
      id: "section-customer-date",
      title: "Customer & Date",
      sequence: 1,
      columns: 2,
      fields: [
        {
          id: "common.customer",
          label: "Customer",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rfq.dates.date",
          label: "Date",
          type: "date",
          size: 1,
          sequence: 2,
          required: false,
        },
      ],
    },
    {
      id: "section-material-specifications",
      title: "Material Specifications",
      sequence: 2,
      columns: 3,
      fields: [
        {
          id: "common.material.materialType",
          label: "Material Type",
          type: "select",
          size: 1,
          sequence: 1,
          required: false,
          options: MATERIAL_TYPE_OPTIONS,
        },
        {
          id: "common.material.materialThickness",
          label: "Material Thickness (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: true,
        },
        {
          id: "common.material.coilWidth",
          label: "Coil Width (in)",
          type: "number",
          size: 1,
          sequence: 3,
          required: true,
        },
        {
          id: "common.material.yieldStrength",
          label: "Yield Strength (psi)",
          type: "number",
          size: 1,
          sequence: 4,
          required: true,
        },
        {
          id: "strUtility.straightener.modulus",
          label: "Elastic Modulus (psi)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
      ],
    },
    {
      id: "section-roll-straightener-specs",
      title: "Roll Straightener Specifications",
      sequence: 3,
      columns: 3,
      fields: [
        {
          id: "common.equipment.straightener.model",
          label: "Straightener Model",
          type: "select",
          size: 1,
          sequence: 1,
          required: false,
          options: STR_MODEL_OPTIONS,
        },
        {
          id: "common.equipment.straightener.rollDiameter",
          label: "Roll Diameter (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.centerDistance",
          label: "Center Distance (in)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.jackForceAvailable",
          label: "Jack Force Available (lbs)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "strUtility.straightener.maxRollDepth",
          label: "Max Roller Depth without Material (in)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.hiddenValue",
          label: "Hidden Calculation Value",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
          defaultValue: 9957.34211927781,
        },
        {
          id: "rollStrBackbend.straightener.rolls.depth.withMaterial",
          label: "Max Roller Depth with Material (in)",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.depthRequired",
          label: "Total Depth Required (in)",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.forceRequired",
          label: "Total Force Required (lbs)",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.yieldMet",
          label: "Yield Requirements Met",
          type: "text",
          size: 1,
          sequence: 10,
          required: false,
        },
      ],
    },
    {
      id: "section-backbend-specifications",
      title: "Backbend Specifications",
      sequence: 4,
      columns: 3,
      fields: [
        {
          id: "rollStrBackbend.straightener.rolls.backbend.radius.comingOffCoil",
          label: "Coming Off Coil Radius (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.radius.offCoilAfterSpringback",
          label: "Off Coil After Springback",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.radius.bendingMomentToYield",
          label: "Bending Moment to Yield Skin (in-lbs)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.radius.oneOffCoil",
          label: "One over radius off coil after springback",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.radius.curveAtYield",
          label: "Curve at Yield",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.radius.radiusAtYield",
          label: "Radius req to yield flat material (in)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
      ],
    },
    {
      id: "section-first-roller-up",
      title: "First Roller - Up Direction",
      sequence: 5,
      columns: 5,
      fields: [
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.height",
          label: "First Roller Height (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.up.resultingRadius",
          label: "Resulting Radius (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.up.curvatureDifference",
          label: "Curvature Difference",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.up.bendingMoment",
          label: "Bending Moment (in-lbs)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.up.bendingMomentRatio",
          label: "Bending Moment Ratio",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.forceRequired",
          label: "Force Required (lbs)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.up.springback",
          label: "Springback",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.up.percentOfThicknessYielded",
          label: "% Thickness Yielded",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.numberOfYieldStrainsAtSurface",
          label: "Yield Strains at Surface",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.up.radiusAfterSpringback",
          label: "Radius After Springback (in)",
          type: "number",
          size: 1,
          sequence: 10,
          required: false,
        },
      ],
    },
    {
      id: "section-first-roller-down",
      title: "First Roller - Down Direction",
      sequence: 6,
      columns: 4,
      fields: [
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.down.resultingRadius",
          label: "Resulting Radius (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.down.curvatureDifference",
          label: "Curvature Difference",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.down.bendingMoment",
          label: "Bending Moment (in-lbs)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.down.bendingMomentRatio",
          label: "Bending Moment Ratio",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.down.springback",
          label: "Springback",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.down.percentOfThicknessYielded",
          label: "% Thickness Yielded",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.first.down.radiusAfterSpringback",
          label: "Radius After Springback (in)",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
      ],
    },
    {
      id: "section-middle-roller-up",
      title: "Middle Roller - Up Direction",
      sequence: 7,
      columns: 4,
      fields: [
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.height",
          label: "Height (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.up.resultingRadius",
          label: "Resulting Radius (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.up.curvatureDifference",
          label: "Curvature Difference",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.up.bendingMoment",
          label: "Bending Moment (in-lbs)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.up.bendingMomentRatio",
          label: "Bending Moment Ratio",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.forceRequired",
          label: "Force Required (lbs)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.up.springback",
          label: "Springback",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.up.percentOfThicknessYielded",
          label: "% Thickness Yielded",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.numberOfYieldStrainsAtSurface",
          label: "Yield Strains at Surface",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.up.radiusAfterSpringback",
          label: "Radius After Springback (in)",
          type: "number",
          size: 1,
          sequence: 10,
          required: false,
        },
      ],
    },
    {
      id: "section-middle-roller-down",
      title: "Middle Roller - Down Direction",
      sequence: 8,
      columns: 4,
      fields: [
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.down.resultingRadius",
          label: "Resulting Radius (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.down.curvatureDifference",
          label: "Curvature Difference",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.down.bendingMoment",
          label: "Bending Moment (in-lbs)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.down.bendingMomentRatio",
          label: "Bending Moment Ratio",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.down.springback",
          label: "Springback",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.down.percentOfThicknessYielded",
          label: "% Thickness Yielded",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.middle.down.radiusAfterSpringback",
          label: "Radius After Springback (in)",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
      ],
    },
    {
      id: "section-last-roller-up",
      title: "Last Roller - Up Direction",
      sequence: 9,
      columns: 5,
      fields: [
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.last.height",
          label: "Last Roller Height (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.last.up.resultingRadius",
          label: "Resulting Radius (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.last.up.curvatureDifference",
          label: "Curvature Difference",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.last.up.bendingMoment",
          label: "Bending Moment (in-lbs)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.last.up.bendingMomentRatio",
          label: "Bending Moment Ratio",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.last.forceRequired",
          label: "Force Required (lbs)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.last.up.springback",
          label: "Springback",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.last.up.percentOfThicknessYielded",
          label: "% Thickness Yielded",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.last.numberOfYieldStrainsAtSurface",
          label: "Yield Strains at Surface",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.last.up.radiusAfterSpringback",
          label: "Radius After Springback (in)",
          type: "number",
          size: 1,
          sequence: 10,
          required: false,
        },
      ],
    },
    {
      id: "section-validation-checks",
      title: "Validation Checks",
      sequence: 6,
      columns: 3,
      fields: [
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.depthRequiredCheck",
          label: "Depth Required Check",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.forceRequiredCheck",
          label: "Force Required Check",
          type: "text",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "rollStrBackbend.straightener.rolls.backbend.rollers.percentYieldCheck",
          label: "Percent Yield Check",
          type: "text",
          size: 1,
          sequence: 3,
          required: false,
        },
      ],
    },
  ],
};

export const FEED_PERFORMANCE_SHEET_SEED = {
  id: "tab-feed",
  label: "Feed",
  value: "feed",
  sequence: 7,
  sections: [
    {
      id: "section-customer-date",
      title: "Customer & Date",
      sequence: 1,
      columns: 2,
      fields: [
        {
          id: "common.customer",
          label: "Customer",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "rfq.dates.date",
          label: "Date",
          type: "date",
          size: 1,
          sequence: 2,
          required: false,
        },
      ],
    },
    {
      id: "section-feed-configuration",
      title: "Feed Configuration",
      sequence: 2,
      columns: 3,
      fields: [
        {
          id: "common.equipment.feed.model",
          label: "Feed Model",
          type: "select",
          size: 1,
          sequence: 1,
          required: true,
          options: FEED_MODEL_OPTIONS,
          default: "CPRF-S1",
        },
        {
          id: "feed.feed.application",
          label: "Application",
          type: "select",
          size: 1,
          sequence: 2,
          required: false,
          options: LINE_APPLICATION_OPTIONS,
          readOnly: true,
        },
        {
          id: "common.equipment.feed.type",
          label: "Feed Type",
          type: "select",
          size: 1,
          sequence: 3,
          required: false,
          options: FEED_TYPE_OPTIONS,
          defaultValue: "Sigma 5",
          readOnly: true,
        },
        {
          id: "feed.feed.feedCheck",
          label: "Feed Check",
          type: "text",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.feed.machineWidth",
          label: "Machine Width",
          type: "select",
          size: 1,
          sequence: 5,
          required: false,
          options: MACHINE_WIDTH_OPTIONS,
          dependsOn: "common.equipment.feed.model",
          dependencyType: "feedMachineWidth",
          default: "6",
        },
        {
          id: "common.equipment.feed.loopPit",
          label: "Loop Pit",
          type: "select",
          size: 1,
          sequence: 6,
          required: false,
          options: YES_NO_OPTIONS,
        },
        {
          id: "feed.feed.fullWidthRolls",
          label: "Full Width Rolls",
          type: "select",
          size: 1,
          sequence: 7,
          required: false,
          options: YES_NO_OPTIONS,
        },
      ],
    },
    {
      id: "section-material-information",
      title: "Material Information",
      sequence: 3,
      columns: 5,
      fields: [
        {
          id: "common.material.coilWidth",
          label: "Width",
          type: "number",
          size: 1,
          sequence: 1,
          required: true,
        },
        {
          id: "common.material.materialThickness",
          label: "Thickness",
          type: "number",
          size: 1,
          sequence: 2,
          required: true,
        },
        {
          id: "common.press.bedLength",
          label: "Press Bed Length",
          type: "number",
          size: 1,
          sequence: 3,
          required: true,
        },
        {
          id: "common.material.materialDensity",
          label: "Density",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.feed.materialInLoop",
          label: "Mat'l In Loop",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
      ],
    },
    {
      id: "section-feed-specifications",
      title: "Feed Specifications",
      sequence: 4,
      columns: 4,
      fields: [
        {
          id: "feed.feed.motor",
          label: "Motor",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.feed.amp",
          label: "AMP",
          type: "text",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.feed.strMaxSpeed",
          label: "STR Max Speed (ft/min)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
          dependsOn: "strUtility.straightener.feedRate",
          dependencyType: "copy",
        },
        {
          id: "feed.feed.frictionInDie",
          label: "Friction in Die (lbs)",
          type: "number",
          size: 1,
          sequence: 4,
          required: true,
        },
        {
          id: "feed.feed.accelerationRate",
          label: "Acceleration Rate (ft/sec²)",
          type: "number",
          size: 1,
          sequence: 5,
          required: true,
        },
        {
          id: "feed.feed.defaultAcceleration",
          label: "Default Accel (ft/sec²)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
          defaultValue: 60,
        },
        {
          id: "feed.feed.maxMotorRPM",
          label: "Max Motor RPM",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
        {
          id: "feed.feed.motorInertia",
          label: "Motor Inertia (lbs-in-sec²)",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
        },
        {
          id: "common.equipment.feed.maxVelocity",
          label: "Max Velocity (ft/min)",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
        },
        {
          id: "feed.feed.settleTime",
          label: "Settle Time (sec)",
          type: "number",
          size: 1,
          sequence: 10,
          required: false,
        },
        {
          id: "feed.feed.ratio",
          label: "Ratio",
          type: "number",
          size: 1,
          sequence: 11,
          required: false,
        },
        {
          id: "feed.feed.chartMinLength",
          label: "Chart Minimum Length (in)",
          type: "number",
          size: 1,
          sequence: 12,
          required: true,
        },
        {
          id: "feed.feed.lengthIncrement",
          label: "Length Increment (in)",
          type: "number",
          size: 1,
          sequence: 13,
          required: true,
        },
        {
          id: "feed.feed.feedAngle1",
          label: "Feed Angle 1 (Deg)",
          type: "select",
          size: 1,
          sequence: 14,
          required: true,
          options: FEED_ANGLE_OPTIONS,
          default: "180",
        },
        {
          id: "feed.feed.feedAngle2",
          label: "Feed Angle 2 (Deg)",
          type: "select",
          size: 1,
          sequence: 15,
          required: true,
          options: FEED_ANGLE_OPTIONS,
          default: "180",
        },
      ],
    },
    {
      id: "section-torque-calculations",
      title: "Torque Calculations",
      sequence: 5,
      columns: 4,
      fields: [
        {
          id: "feed.feed.regen",
          label: "ReGen (Watts)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "feed.feed.torque.motorPeak",
          label: "Motor Peak Torque (lbs-in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "feed.feed.torque.rms.motor",
          label: "Motor RMS Torque (lbs-in)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "feed.feed.torque.frictional",
          label: "Frictional Torque (lbs-in)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "feed.feed.torque.loop",
          label: "Loop Torque (lbs-in)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "feed.feed.torque.settle",
          label: "Settle Torque (lbs-in)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "feed.feed.reflInertia",
          label: "REF. Inertia (lbs-in-sec²)",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
        {
          id: "feed.feed.match",
          label: "MATCH",
          type: "number",
          size: 1,
          sequence: 8,
          required: false,
        },
        {
          id: "feed.feed.torque.peak",
          label: "Peak Torque (lbs-in)",
          type: "number",
          size: 1,
          sequence: 9,
          required: false,
        },
        {
          id: "feed.feed.torque.rms.feedAngle1",
          label: "RMS Torque (FA1) (lbs-in)",
          type: "number",
          size: 1,
          sequence: 10,
          required: false,
        },
        {
          id: "feed.feed.torque.rms.feedAngle2",
          label: "RMS Torque (FA2) (lbs-in)",
          type: "number",
          size: 1,
          sequence: 11,
          required: false,
        },
        {
          id: "feed.feed.torque.acceleration",
          label: "Acceleration Torque (lbs-in)",
          type: "number",
          size: 1,
          sequence: 12,
          required: false,
        },
      ],
    },
    {
      id: "section-pull-through",
      title: "Pull-Through Configuration",
      sequence: 6,
      columns: 4,
      conditionalVisibility: {
        dependsOn: "common.equipment.feed.type",
        showWhen: "Sigma 5 w/ Pull Through"
      },
      fields: [
        {
          id: "feed.feed.pullThru.centerDistance",
          label: "Center Distance (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
          conditional: {
            dependsOn: "common.equipment.feed.type",
            showWhen: "Sigma 5 w/ Pull Through"
          },
        },
        {
          id: "feed.feed.pullThru.yieldStrength",
          label: "Yield Strength (psi)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          conditional: {
            dependsOn: "common.equipment.feed.type",
            showWhen: "Sigma 5 w/ Pull Through"
          },
        },
        {
          id: "feed.feed.pullThru.kConst",
          label: "K-Constant",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
          conditional: {
            dependsOn: "common.equipment.feed.type",
            showWhen: "Sigma 5 w/ Pull Through"
          },
        },
        {
          id: "feed.feed.pullThru.pinchRolls",
          label: "Straightener Pinch Rolls",
          type: "select",
          size: 1,
          sequence: 4,
          required: false,
          options: STRAIGHTENER_ROLLS_OPTIONS,
          conditional: {
            dependsOn: "common.equipment.feed.type",
            showWhen: "Sigma 5 w/ Pull Through"
          },
        },
      ],
    },
    {
      id: "section-feed-checks",
      title: "Validation Checks",
      sequence: 10,
      columns: 3,
      fields: [
        {
          id: "feed.feed.matchCheck",
          label: "Match Check",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "feed.feed.torque.peakCheck",
          label: "Peak Torque Check",
          type: "text",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "feed.feed.torque.accelerationCheck",
          label: "Acceleration Check",
          type: "text",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "feed.feed.torque.rms.motorCheck",
          label: "Motor Check",
          type: "text",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "feed.feed.torque.rms.feedAngle1Check",
          label: "Feed Angle 1 Check",
          type: "text",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "feed.feed.torque.rms.feedAngle2Check",
          label: "Feed Angle 2 Check",
          type: "text",
          size: 1,
          sequence: 6,
          required: false,
        },
      ],
    },
    {
      id: "section-feed-performance",
      title: "Feed Performance Data",
      sequence: 7,
      columns: 1,
      fields: [
        {
          id: "feed.feed.tableValues",
          label: "Feed Performance Table & Graph",
          type: "custom",
          size: 2,
          sequence: 1,
          required: false,
          readOnly: true,
        },
      ],
    },
  ],
};

export const SHEAR_PERFORMANCE_SHEET_SEED = {
  id: "tab-shear",
  label: "Shear",
  value: "shear",
  sequence: 8,
  sections: [
    {
      id: "section-customer-date",
      title: "Customer & Date",
      sequence: 1,
      columns: 2,
      fields: [
        {
          id: "common.customer",
          label: "Customer",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "common.date",
          label: "Date",
          type: "date",
          size: 1,
          sequence: 2,
          required: false,
        },
      ],
    },
    {
      id: "section-material-specs",
      title: "Material Specifications",
      sequence: 2,
      columns: 2,
      fields: [
        {
          id: "shear.material.thickness",
          label: "Material Thickness (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "shear.material.width",
          label: "Material Width (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "shear.material.tensileStrength",
          label: "Tensile Strength (PSI)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "shear.material.shearStrength",
          label: "Shear Strength (PSI)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "shear.material.clearancePercent",
          label: "Clearance (%)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "shear.material.bladeAngle",
          label: "Blade Angle (°)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
        {
          id: "shear.material.penetration",
          label: "Penetration (in)",
          type: "number",
          size: 1,
          sequence: 7,
          required: false,
        },
        {
          id: "shear.material.type",
          label: "Shear Type",
          type: "select",
          size: 1,
          sequence: 8,
          required: false,
          options: SHEAR_TYPE_OPTIONS,
        },
      ],
    },
    {
      id: "section-blade-specs",
      title: "Blade Specifications",
      sequence: 3,
      columns: 2,
      fields: [
        {
          id: "shear.blade.effectiveLength",
          label: "Effective Blade Length (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "shear.blade.clearance",
          label: "Clearance (in)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "shear.blade.angleRadians",
          label: "Blade Angle (radians)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "shear.blade.normalLoad",
          label: "Normal Load (lbs)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "shear.blade.sideLoad",
          label: "Side Load (lbs)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
        {
          id: "shear.blade.totalLoad",
          label: "Total Load (lbs)",
          type: "number",
          size: 1,
          sequence: 6,
          required: false,
        },
      ],
    },
    {
      id: "section-cylinder-specs",
      title: "Cylinder Specifications",
      sequence: 4,
      columns: 2,
      fields: [
        {
          id: "shear.cylinder.bore",
          label: "Cylinder Bore (in)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "shear.cylinder.area",
          label: "Cylinder Area (in²)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "shear.cylinder.forceNeeded",
          label: "Force Needed (lbs)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "shear.cylinder.numberOfCylinders",
          label: "Number of Cylinders",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
        {
          id: "shear.cylinder.forcePerCylinder",
          label: "Force per Cylinder (lbs)",
          type: "number",
          size: 1,
          sequence: 5,
          required: false,
        },
      ],
    },
    {
      id: "section-hydraulic-pressure",
      title: "Hydraulic Pressure",
      sequence: 5,
      columns: 2,
      fields: [
        {
          id: "shear.hydraulic.normalPressure",
          label: "Normal Pressure (PSI)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "shear.hydraulic.sidePressure",
          label: "Side Pressure (PSI)",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "shear.hydraulic.totalPressure",
          label: "Total Pressure (PSI)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
      ],
    },
    {
      id: "section-time",
      title: "Time",
      sequence: 6,
      columns: 2,
      fields: [
        {
          id: "shear.time.cycleTime",
          label: "Cycle Time (sec)",
          type: "number",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "shear.time.cutsPerMinute",
          label: "Cuts per Minute",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
        },
        {
          id: "shear.time.cutTime",
          label: "Cut Time (sec)",
          type: "number",
          size: 1,
          sequence: 3,
          required: false,
        },
        {
          id: "shear.time.returnTime",
          label: "Return Time (sec)",
          type: "number",
          size: 1,
          sequence: 4,
          required: false,
        },
      ],
    },
    {
      id: "section-conclusions",
      title: "Conclusions",
      sequence: 7,
      columns: 2,
      fields: [
        {
          id: "shear.shear.conclusions.force.requiredToShearCheck",
          label: "Required to Shear Check",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
        },
        {
          id: "shear.conclusions.notes",
          label: "Notes",
          type: "textarea",
          size: 2,
          sequence: 2,
          required: false,
        },
      ],
    },
  ],
};

export const SUMMARY_REPORT_PERFORMANCE_SHEET_SEED = {
  id: "tab-summary-report",
  label: "Summary Report",
  value: "summary-report",
  sequence: 9,
  sections: [
    {
      id: "section-customer-date",
      title: "Customer & Date",
      sequence: 1,
      columns: 2,
      fields: [
        {
          id: "common.customer",
          label: "Customer",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "rfq.dates.date",
          label: "Date",
          type: "date",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-reel",
      title: "Reel",
      sequence: 2,
      columns: 2,
      fields: [
        {
          id: "common.equipment.reel.model",
          label: "Reel Model",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.reel.width",
          label: "Reel Width",
          type: "text",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },


        {
          id: "common.equipment.reel.backplate.diameter",
          label: "Backplate Diameter",
          type: "text",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.motorization.isMotorized",
          label: "Reel Motorization",
          type: "text",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "materialSpecs.reel.style",
          label: "Single or Double Ended",
          type: "text",
          size: 1,
          sequence: 5,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-threading-drive",
      title: "Threading Drive",
      sequence: 3,
      columns: 2,
      fields: [
        {
          id: "tddbhd.reel.threadingDrive.airClutch",
          label: "Air Clutch",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.reel.threadingDrive.hydThreadingDrive",
          label: "Hyd. Threading Drive",
          type: "text",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-hold-down",
      title: "Hold Down",
      sequence: 4,
      columns: 2,
      fields: [
        {
          id: "tddbhd.scenarios[0].reel.holddown.assy",
          label: "Hold Down Assy",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.scenarios[0].reel.holddown.cylinderPressure",
          label: "Hold Down Cylinder",
          type: "text",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-drag-brake",
      title: "Drag Brake",
      sequence: 5,
      columns: 2,
      fields: [
        {
          id: "tddbhd.scenarios[0].reel.dragBrake.model",
          label: "Brake Model",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "tddbhd.scenarios[0].reel.dragBrake.quantity",
          label: "Brake Quantity",
          type: "text",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
      ],
    },

    {
      id: "section-motorized-reel",
      title: "Motorized Reel",
      sequence: 6,
      columns: 2,
      fields: [
        {
          id: "reelDrive.reel.motorization.driveHorsepower",
          label: "Drive Horsepower",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.motorization.speed",
          label: "Speed (ft/min)",
          type: "text",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.motorization.accelRate",
          label: "Accel Rate (ft/sec²)",
          type: "text",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "reelDrive.reel.motorization.regenRequired",
          label: "Regen Req'd",
          type: "text",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-powered-straightener",
      title: "Powered Straightener",
      sequence: 7,
      columns: 2,
      fields: [
        {
          id: "common.equipment.straightener.model",
          label: "Straightener Model",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.straightener.numberOfRolls",
          label: "Straightening Rolls",
          type: "number",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "strUtility.straightener.payoff",
          label: "Payoff",
          type: "text",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.straightener.width",
          label: "Str. Width (in)",
          type: "text",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "strUtility.scenarios[0].straightener.feedRate",
          label: "Feed Rate (ft/min)",
          type: "text",
          size: 1,
          sequence: 5,
          required: false,
          readOnly: true,
        },
        {
          id: "strUtility.scenarios[0].straightener.acceleration",
          label: "Acceleration (ft/sec)",
          type: "text",
          size: 1,
          sequence: 6,
          required: false,
          readOnly: true,
        },
        {
          id: "strUtility.scenarios[0].straightener.horsepower",
          label: "Horsepower (HP)",
          type: "text",
          size: 1,
          sequence: 7,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-sigma-5-feed",
      title: "Sigma 5 Feed",
      sequence: 8,
      columns: 2,
      fields: [
        {
          id: "feed.feed.application",
          label: "Application",
          type: "text",
          size: 1,
          sequence: 1,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.feed.model",
          label: "Model",
          type: "text",
          size: 1,
          sequence: 2,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.feed.machineWidth",
          label: "Machine Width",
          type: "text",
          size: 1,
          sequence: 3,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.feed.loopPit",
          label: "Loop Pit",
          type: "text",
          size: 1,
          sequence: 4,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.feed.fullWidthRolls",
          label: "Full Width Rolls",
          type: "text",
          size: 1,
          sequence: 5,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.scenarios[0].feed.feedAngle1",
          label: "Feed Angle 1",
          type: "text",
          size: 1,
          sequence: 6,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.scenarios[0].feed.feedAngle2",
          label: "Feed Angle 2",
          type: "text",
          size: 1,
          sequence: 7,
          required: false,
          readOnly: true,
        },
        {
          id: "common.press.bedLength",
          label: "Press Bed Length",
          type: "text",
          size: 1,
          sequence: 8,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.feed.maxVelocity",
          label: "Maximum Velocity ft/min",
          type: "text",
          size: 1,
          sequence: 9,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.scenarios[0].feed.accelerationRate",
          label: "Acceleration (ft/sec²)",
          type: "text",
          size: 1,
          sequence: 10,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.scenarios[0].feed.ratio",
          label: "Ratio",
          type: "text",
          size: 1,
          sequence: 11,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.feed.pullThru.straightenerRolls",
          label: "Pull Thru Straightener Rolls",
          type: "text",
          size: 1,
          sequence: 12,
          required: false,
          readOnly: true,
        },
        {
          id: "feed.feed.pullThru.pinchRolls",
          label: "Pull Thru Pinch Rolls",
          type: "text",
          size: 1,
          sequence: 13,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.feed.direction",
          label: "Feed Direction",
          type: "text",
          size: 1,
          sequence: 14,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.feed.controlsLevel",
          label: "Controls Level",
          type: "text",
          size: 1,
          sequence: 15,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.feed.typeOfLine",
          label: "Type of line",
          type: "text",
          size: 1,
          sequence: 16,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.feed.passline",
          label: "Passline",
          type: "text",
          size: 1,
          sequence: 17,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.feed.lightGuageNonMarking",
          label: "Light Gauge Non-Marking",
          type: "checkbox",
          size: 1,
          sequence: 18,
          required: false,
          readOnly: true,
        },
        {
          id: "common.equipment.feed.nonMarking",
          label: "Non-Marking",
          type: "checkbox",
          size: 1,
          sequence: 19,
          required: false,
          readOnly: true,
        },
      ],
    },
    {
      id: "section-performance-results",
      title: "Feed Performance Data",
      sequence: 9,
      columns: 1,
      fields: [
        {
          id: "feed.scenarios[0].feed.tableValues",
          label: "Feed Performance Table & Graph",
          type: "custom",
          size: 2,
          sequence: 1,
          required: false,
          readOnly: true,
        },
      ],
    },
  ],
};
