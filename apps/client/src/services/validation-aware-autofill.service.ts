...
import { AutofillTriggerService } from "./autofill-trigger.service";
import { VisibleTab } from "@/utils/tab-visibility";
...
export class ValidationAwareAutofillService {
  ...
  private static readonly ENGINEERING_DEFAULTS: EngineeringDefaults = {
      ...
      'feed.feed.pullThru.isPullThru': {
        value: (data: PerformanceData) => {
          ...
          return (lineType === 'Compact' ||
            (typeOfLine && typeOfLine.toLowerCase().includes('pull through'))) ? 'Yes' : 'No';
        },
        condition: () => true,
        priority: 85
      },
    ...
    'summary-report': {
      ...
      'common.equipment.reel.model': {
        value: (data: PerformanceData) => {
          const width = parseFloat(data?.common?.material?.coilWidth?.toString() || '48');
          const weight = parseFloat(data?.common?.coil?.maxCoilWeight?.toString() || '4000');
          // Select reel model based on material width and coil weight
          if (width <= 12 && weight <= 5000) return 'CPR-040';
          if (width <= 24 && weight <= 10000) return 'CPR-060';
          if (width <= 36 && weight <= 15000) return 'CPR-080';
          if (width <= 48 && weight <= 20000) return 'CPR-100';
          if (width <= 60 && weight <= 30000) return 'CPR-150';
      ...
      'common.equipment.reel.width': {
        value: (data: PerformanceData) => {
          const coilWidth = parseFloat(data?.common?.material?.coilWidth?.toString() || '48');
      ...
      'common.equipment.reel.backplate.diameter': {
        value: (data: PerformanceData) => {
          const coilID = parseFloat(data?.common?.coil?.coilID?.toString() || '20');
    ...
    'material-specs': {
      'common.material.materialDensity': {
        value: (data: PerformanceData) => {
          const materialType = data?.common?.material?.materialType?.toLowerCase() || '';
          if (materialType.includes('steel')) return 0.284;
          if (materialType.includes('aluminum')) return 0.098;
          if (materialType.includes('stainless')) return 0.289;
      ...
      'materialSpecs.material.minBendRadius': {
        value: (data: PerformanceData) => {
          const thickness = data?.common?.material?.materialThickness || 0;
    ...
    'str-utility': {
      'common.equipment.straightener.model': {
        value: (data: PerformanceData) => {
          const width = parseFloat(data?.common?.material?.coilWidth?.toString() || '48');
          const thickness = parseFloat(data?.common?.material?.materialThickness?.toString() || '0.125');
      ...
      'strUtility.straightener.feedRate': {
        value: (data: PerformanceData) => {
          const avgSpm = data?.common?.feedRates?.average?.spm || 30;
          const avgLength = data?.common?.feedRates?.average?.length || 12;
    ...
    'feed': {
      ...
      'feed.feed.motor.hp': {
        value: (data: PerformanceData) => {
          const reelSpeed = parseFloat(data?.reelDrive?.reel?.motorization?.speed?.toString() || '400');
          const coilWeight = parseFloat(data?.common?.coil?.maxCoilWeight?.toString() || '30000');
          const width = parseFloat(data?.common?.material?.coilWidth?.toString() || '3');
      ...
      'feed.feed.feedConfiguration': {
        value: (data: PerformanceData) => {
          const thickness = parseFloat(data?.common?.material?.materialThickness?.toString() || '0.07');
          if (thickness <= 0.05) return 'Light Gauge Configuration';
          if (thickness <= 0.125) return 'Medium Gauge Configuration';
      ...
      'feed.feed.feedRolls.grip.pressure': {
        value: (data: PerformanceData) => {
          const yieldStrength = parseFloat(data?.common?.material?.maxYieldStrength?.toString() || '45000');
          const thickness = parseFloat(data?.common?.material?.materialThickness?.toString() || '0.07');
      ...
      'feed.feed.threading.threadingSpeed': {
        value: (data: PerformanceData) => {
          const normalSpeed = parseFloat(data?.reelDrive?.reel?.motorization?.speed?.toString() || '400');
    ...
    'reel-drive': {
      'reelDrive.reel.motorization.driveHorsepower': {
        value: (data: PerformanceData) => {
          const coilWeight = data?.common?.coil?.maxCoilWeight || 5000;
          const speed = data?.reelDrive?.reel?.motorization?.speed || 100;
    ...
    'tddbhd': {
      'tddbhd.coil.coilOD': {
        value: (data: PerformanceData) => parseFloat(data?.common?.coil?.maxCoilOD?.toString() || '60'),
        condition: (data) => !!data?.common?.coil?.maxCoilOD,
        priority: 90
      },
      'tddbhd.coil.coilWeight': {
        value: (data: PerformanceData) => parseFloat(data?.common?.coil?.maxCoilWeight?.toString() || '4000'),
        condition: (data) => !!data?.common?.coil?.maxCoilWeight,
        priority: 90
      },
      ...
      'tddbhd.reel.dragBrake.torque': {
        value: (data: PerformanceData) => {
          const coilWeight = parseFloat(data?.common?.coil?.maxCoilWeight?.toString() || '4000');
          const coilOD = parseFloat(data?.common?.coil?.maxCoilOD?.toString() || '60');
      ...
      'tddbhd.reel.dragBrake.model': {
        value: (data: PerformanceData) => {
          ...
          const coilWeight = parseFloat(data?.common?.coil?.maxCoilWeight?.toString() || '2000');
          const coilOD = parseFloat(data?.common?.coil?.maxCoilOD?.toString() || '60');
          const thickness = parseFloat(data?.common?.material?.materialThickness?.toString() || '0.07');
      ...
      'tddbhd.reel.cylinderBore': {
        value: (data: PerformanceData) => {
          ...
          if (brakeModel.includes('Single Stage')) return 5;
          if (brakeModel.includes('Double Stage')) return 4;
          if (brakeModel.includes('Triple Stage')) return 4;
          return 5; // Default
        },
        condition: () => true,
        priority: 70
      ...
      'tddbhd.reel.brakePadDiameter': {
        value: (data: PerformanceData) => {
          const reelModel = data?.common?.equipment?.reel?.model || '';
          if (reelModel.includes('040')) return 12;
          if (reelModel.includes('150')) return 16;
          return 14;
        },
        condition: (data) => !!data?.common?.equipment?.reel?.model,
        priority: 75
      ...
      'tddbhd.reel.holddown.force.available': {
        value: (data: PerformanceData) => {
          const required = Math.round(parseFloat(data?.common?.coil?.maxCoilWeight?.toString() || '4000') * 0.15);
    ...
    'roll-str-backbend': {
      ...
      'rollStrBackbend.straightener.jackForceAvailable': {
        value: (data: PerformanceData) => {
          ...
          if (strModel.includes('250')) return 25000;
          if (strModel.includes('306')) return 50000;
    ...
    'shear': {
      'shear.shear.hydraulic.pressure': {
        value: (data: PerformanceData) => {
          const thickness = data?.common?.material?.materialThickness || 0.125;
          const tensileStrength = data?.common?.material?.maxTensileStrength || data?.common?.material?.maxYieldStrength || 50000;
  ...
  private static readonly VALIDATION_RULES: ValidationRule[] = [
    ...
    },
    {
      ...
      suggest: (data) => {
        const materialType = data?.common?.material?.materialType?.toLowerCase() || '';
        if (materialType.includes('steel')) return 50000;
        if (materialType.includes('aluminum')) return 35000;
        if (materialType.includes('stainless')) return 75000;
        return 50000;
      },
  ...
  public static getValidationAwareAutofill(
    ...
    for (const tabName of visibleTabNames) {
      ...
      if (!tabDefaults) continue;

      for (const [fieldPath, defaultConfig] of Object.entries(tabDefaults)) {
        // Check if field is already populated
        const currentValue = AutofillTriggerService.getNestedValue(data, fieldPath);
        if (AutofillTriggerService.hasMeaningfulValue(currentValue)) continue;

        // Check condition if specified
        if (defaultConfig.condition && !defaultConfig.condition(data)) continue;
    ...
    for (const rule of this.VALIDATION_RULES) {
      const currentValue = AutofillTriggerService.getNestedValue(data, rule.fieldPath);
      if (!AutofillTriggerService.hasMeaningfulValue(currentValue) || !rule.validate(currentValue, data)) {
  ...
  public static getPassingCalculationValues(
    ...
    if (!tabDefaults) return suggestions;
