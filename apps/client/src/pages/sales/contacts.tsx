...
import { AddContactModal, Button, PageHeader, Table } from "@/components";
...
function Contacts() {
  ...
  const processContactsResponse = async (rawContacts: any) => {
    ...
    const uniqueCompanyIds = [...new Set(validContacts.map((c: any) => c.legacyCompanyId).filter(Boolean))].map((id: any) => Number.parseInt(id, 10));
    const companyNameMap = await fetchCompanyNames(uniqueCompanyIds);

    const mapped = validContacts.map((contact: any) => {
      const companyName = contact.legacyCompanyId ? companyNameMap.get(Number.parseInt(contact.legacyCompanyId, 10)) : undefined;
    ...
    },
    {
      ...
      render: (_, row) => (
        <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${
          row.type === ContactType.Accounting
            ? "bg-blue-100 text-blue-800"
            : row.type === ContactType.Engineering
              ? "bg-green-100 text-green-800"
              : row.type === ContactType.Sales
                ? "bg-orange-100 text-orange-800"
                : row.type === ContactType.Parts_Service
                  ? "bg-purple-100 text-purple-800"
                  : row.type === ContactType.Inactive
                    ? "bg-gray-100 text-gray-800"
                    : row.type === ContactType.Left_Company
...
function ContactsMapView({
  ...
  const batchLookupPostalCodes = async (postalCodes: Array<{ country: string; postalCode: string }>) => {
    ...
    for (let i = 0; i < uncachedCodes.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async (key) => {
        ...
        if (postalCodeCacheRef.current.has(cacheKey))
          return;
        ...
        try {
          ...
          if (result && Array.isArray(result) && result.length > 0) {
            ...
            if (zipData.Latitude && zipData.Longitude) {
              const lat = Number.parseFloat(zipData.Latitude);
              const lng = Number.parseFloat(zipData.Longitude);
  ...
  useEffect(() => {
    if (!mapContainerRef.current || contactsWithAddresses.length === 0 || !isInitialLoad)
      return;
