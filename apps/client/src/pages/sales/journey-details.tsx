...
import { AddContactModal, Button, Modal, PageHeader, Table, Tabs } from "@/components";
...
import { STAGES, VALID_CONFIDENCE_LEVELS, VALID_DEALER_CONTACTS, VALID_DEALERS, VALID_EQUIPMENT_TYPES, VALID_INDUSTRIES, VALID_JOURNEY_TYPES, VALID_LEAD_SOURCES, VALID_PRESENTATION_METHODS, VALID_QUOTE_TYPES, VALID_REASON_LOST, VALID_REASON_WON } from "./journeys/constants";
import { COMPETITION_OPTIONS } from "./journeys/types";
import { fetchAvailableRsms, fetchDemographicCategory, formatDateForDatabase, getValidDealer, getValidDealerContact, getValidEquipmentType, getValidIndustry, getValidJourneyType, getValidLeadSource } from "./journeys/utils";
...
function getStageLabel(journey: any) {
...
async function logJourneyChanges(api: any, journeyId: string, oldData: any, newData: Record<string, any>, originalUpdates: Record<string, any>, employee: any) {
...
async function saveJourneyUpdates(api: any, journey: any, updates: Record<string, any>, originalUpdates: Record<string, any>, employee: any, setIsSaving: (val: boolean) => void) {
...
function extractDateOnly(dateStr: any) {
  if (!dateStr)
    return "";
...
function EditButtons({ isEditing, onSave, onCancel, onEdit, isSaving }: any) {
  return (
...
function JourneyDetailsTab({ journey, journeyContacts, updateJourney, setJourneyContacts, employee, validJourneyStatuses }: { journey: any | null; journeyContacts: any[]; updateJourney: (updates: Record<string, any>) => void; setJourneyContacts: React.Dispatch<React.SetStateAction<any[]>>; employee: any; validJourneyStatuses: string[] }) {
  ...
  const handleCreateNote = async () => {
    if (!newNoteBody.trim() || !journey?.ID && !journey?.id)
      return;
  ...
  const handleCreateNextStep = async () => {
    if (!newNextStepBody.trim() || !newNextStepDate || !journey?.ID && !journey?.id)
      return;
  ...
  const handleSaveNextStep = async () => {
    if (!editingNextStepId || !editingNextStepBody.trim())
      return;
    setIsSaving(true);
    try {
  ...
  const handleSaveDetails = async () => {
    const baseUpdates = {
      ...
      RSM_Helped: detailsForm.rsmAssist,
                          ...
                          ? (
                              <>
                                ...
                                <div className="text-xs text-text-muted mt-1">
                                  Selected ID:
                                  {" "}
                                  <span className="font-mono">{customerForm.companyId || "None"}</span>
                                </div>
                              </>
                            )
                          : (
                                          ...
                                          <>
                                            ...
                                            {contact.Contact_Email && (
                                              <div className="text-xs text-text-muted mb-1">
                                                ...
                                                {" "}
                                                <a href={`mailto:${contact.Contact_Email}`} className="text-primary hover:underline">{contact.Contact_Email}</a>
                                              </div>
                                            )}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <div className="absolute bottom-full left-0 mb-1 px-2 py-1 bg-black text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-10">
                            Priority:
                            {" "}
                            {journey?.Priority || journey?.priority || "None"}
                            {" "}
                            (
                            {getPriorityLabel(journey?.Priority || journey?.priority)}
                            )
                          </div>
                        </div>
                        <div className="text-sm text-text">
                                  ...
                                  <div className="flex justify-between items-start mb-2">
                                    <div className="flex flex-col">
                                      ...
                                      <span className="text-xs text-text-muted">
                                        Created:
                                        {" "}
                                        {nextStep.createdAt ? new Date(nextStep.createdAt).toLocaleString() : "N/A"}
                                      </span>
                                      {nextStep.updatedAt && nextStep.updatedAt !== nextStep.createdAt && (
                                        <span className="text-xs text-text-muted">
                                          Updated:
                                          {" "}
                                          {new Date(nextStep.updatedAt).toLocaleString()}
                                        </span>
                                      )}
                                    </div>
                                    <div className="flex items-center gap-2">
            ...
            {contactToDelete?._isPrisma
              ? (
                  <>
                    ...
                    {contactToDelete?.Contact_Name || "Unnamed Contact"}
                    " from this journey?
                  </>
                )
              : (
                  <>
                    Are you sure you want to delete the legacy contact "
                    {contactToDelete?.Contact_Name || "Unnamed Contact"}
                    "?
                  </>
                )}
    ...
    <div className="flex flex-1 flex-col p-2 md:p-4">
      <div className="bg-foreground rounded shadow-sm border p-2 flex-1 overflow-x-auto">
        <div className="text-xs font-bold text-text-muted mb-1">
          Log Records
          {" "}
          {loadingLogs && <span className="text-text-muted">(Loading...)</span>}
        </div>
        <Table
          columns={[
            {
...
function JourneyTagsTab({ journey, employee }: { journey: any | null; employee: any }) {
  ...
  const handleAddTag = async () => {
    if (!newTagInput.trim() || !journey?.ID && !journey?.id)
      return;
...
function JourneyDetailsPage() {
  ...
  if (!journeyId)
    return <div className="w-full flex flex-1 flex-col"><PageHeader title="Invalid Journey" description="No journey ID provided in the URL." goBack /></div>;
  if (!journeyData)
    return <div className="w-full flex flex-1 flex-col"><PageHeader title="Journey not found" description="This journey may have been removed or is unavailable." goBack /></div>;
