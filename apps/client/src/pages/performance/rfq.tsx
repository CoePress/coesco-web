...
const getVersioned = (ps: PerformanceSheetState, key: 'maxThick' | 'atFull' | 'minThick' | 'atWidth') => ps[key];
...
const RFQ = () => {
  ...
  async function triggerCalculationRFQ(versionKey: string, versionData: any) {
    const coilWeight = versionData.coilWeight !== undefined && versionData.coilWeight !== null && versionData.coilWeight !== ''
    ...
    try {
      ...
      const result = await pythonInstance.post('/material_specs/calculate_variant', payload);
      setPerformanceSheet((prev) => ({
        ...prev,
        [versionKey]: {
  ...
  useEffect(() => {
    fetchFPM(performanceSheet.avgFeedLen, performanceSheet.avgFeedSPM, 'avgFPM');
  }, [performanceSheet.avgFeedLen, performanceSheet.avgFeedSPM]);
  useEffect(() => {
    fetchFPM(performanceSheet.maxFeedLen, performanceSheet.maxFeedSPM, 'maxFPM');
  }, [performanceSheet.maxFeedLen, performanceSheet.maxFeedSPM]);
  useEffect(() => {
    fetchFPM(performanceSheet.minFeedLen, performanceSheet.minFeedSPM, 'minFPM');
  ...
  const handleChange = (e: any) => {
    ...
    if (name.includes("pressType.")) {
      ...
      if (
        ...
        updatePerformanceSheet({
          [spec]: {
            ...(performanceSheet[spec as 'maxThick' | 'atFull' | 'minThick' | 'atWidth']),
            [field]: value,
          },
        });
      }
    ...
    <div className="w-full flex flex-1 flex-col p-2 gap-2">
      <Card className="mb-0 p-4">
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="flex items-end gap-2">
            ...
            <Button 
              as="button" 
              onClick={() => createRFQ(performanceSheet)}
              disabled={isLoading}
            >
      ...
      <Card className="mb-0 p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          />
          <Input label="Current brand of feed equipment" name="feedEquipment" value={performanceSheet.feedEquipment} onChange={handleChange} />
        </div>
      </Card>
      ...
      <Card className="mb-0 p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          {performanceSheet.pressType.other && <Input label="Other..." name="pressType.otherText" value={performanceSheet.pressType.otherText} onChange={handleChange} />}
        </div>
