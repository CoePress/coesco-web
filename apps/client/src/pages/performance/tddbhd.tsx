...
import { MATERIAL_TYPE_OPTIONS, YES_NO_OPTIONS, REEL_MODEL_OPTIONS, FEED_DIRECTION_OPTIONS,
        REEL_WIDTTH_OPTIONS, BACKPLATE_DIAMETER_OPTIONS, HYDRAULIC_THREADING_DRIVE_OPTIONS,
        HOLD_DOWN_ASSY_OPTIONS, HOLD_DOWN_CYLINDER_OPTIONS, BRAKE_MODEL_OPTIONS, BRAKE_QUANTITY_OPTIONS
...
const emptyReel = { reelModel: '', width: '', backplate: '', materialType: '', materialWidth: '', thickness: '', yieldStrength: '', airPressure: '', decelRate: '' };
...
const emptyDragBrake = { reelModel: '', quantity: '', torqueRequired: '', failsafePSI: '', failsafeHoldingForce: '' };
...
export function mapBackendToFrontendTDDBHD(backendData: any, prevTddbhd?: Partial<TDDBHDData>): TDDBHDData {
  const versionMap = [
  ...
  const referenceNumber = backendData.reference || backendData.referenceNumber || prevTddbhd?.referenceNumber || '';
  return {
    referenceNumber,
    customer: backendData.customer || backendData.customer || prevTddbhd?.customer || '',
...
export default function TDDBHD() {
  ...
  function handleInputChange(
    section: 'reel' | 'coil' | 'brake' | 'threadingDrive' | 'holdDown' | 'cylinder' | 'dragBrake',
    field: string,
    value: string
  ) {
    ...
    try {
      ...
      if (backendData) {
        ...
        setPerformanceSheet(prev => ({
          ...
          tddbhd: {
            ...tddbhd,
            referenceNumber: backendData.reference || tddbhd.referenceNumber || '',
            customer: backendData.customer || backendData.customer || tddbhd.customer || '',
  ...
  // Use tddbhd state for all fields with safe fallback
    ...
    <div className="w-full flex flex-1 flex-col p-2 gap-2">
      ...
      <Card className="mb-0 p-4">
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="flex items-end gap-2">
            ...
            <Button as="button" onClick={handleSetData} disabled={isSaving}>Send Data</Button>
          </div>
        </div>
      </Card>
      ...
      <Card className="mb-0 p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <Input label="Reel Model"
              value={data.reel['Maximum Thick']?.reelModel || ''}
              onChange={e => handleInputChange('reel', 'reelModel', e.target.value)}
            />
            <Select
              label="Reel Width"
              value={data.reel['Maximum Thick']?.width || ''}
              onChange={e => handleInputChange('reel', 'width', e.target.value)}
              options={REEL_WIDTTH_OPTIONS.map(opt => ({ value: opt.value, label: opt.Label }))}
            />
            <Select
              label="Backplate Diameter"
              value={data.reel['Maximum Thick']?.backplate || ''}
              onChange={e => handleInputChange('reel', 'backplate', e.target.value)}
              options={BACKPLATE_DIAMETER_OPTIONS.map(opt => ({ value: opt.value, label: opt.Label }))}
            />
            <Select
              label="Material Type"
            ...
            />
            <Input label="Material Width (in)" value={data.reel['Maximum Thick']?.materialWidth || ''} onChange={e => handleInputChange('reel', 'materialWidth', e.target.value)} />
            <Input label="Material Thickness (in)" value={data.reel['Maximum Thick']?.thickness || ''} onChange={e => handleInputChange('reel', 'thickness', e.target.value)} />
            <Input label="Material Yield Strength (psi)" value={data.reel['Maximum Thick']?.yieldStrength || ''} onChange={e => handleInputChange('reel', 'yieldStrength', e.target.value)} />
            <Input label="Air Pressure Available (psi)" value={data.reel['Maximum Thick']?.airPressure || ''} onChange={e => handleInputChange('reel', 'airPressure', e.target.value)} />
            <Input label="Required Decel. Rate (ft/sec²)" value={data.reel['Maximum Thick']?.decelRate || ''} onChange={e => handleInputChange('reel', 'decelRate', e.target.value)} />
          </div>
        <Card className="mb-4 p-4">
          <Text as="h3" className="mb-4 text-lg font-medium">Coil, Brake & Other Specs</Text>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Input label="Coil Weight (lbs)" value={data.coil['Maximum Thick']?.weight || ''} onChange={e => handleInputChange('coil', 'weight', e.target.value)} />
            <Input label="Coil O.D. (in)" value={data.coil['Maximum Thick']?.od || ''} onChange={e => handleInputChange('coil', 'od', e.target.value)} />
            <Input label="Disp. (Reel) Mtr." value={data.coil['Maximum Thick']?.dispReel || ''} onChange={e => handleInputChange('coil', 'dispReel', e.target.value)} />
            <Input label="Web Tension (psi)" value={data.coil['Maximum Thick']?.webTensionPsi || ''} onChange={e => handleInputChange('coil', 'webTensionPsi', e.target.value)} />
            <Input label="Web Tension (lbs)" value={data.coil['Maximum Thick']?.webTensionLbs || ''} onChange={e => handleInputChange('coil', 'webTensionLbs', e.target.value)} />
            <Input label="Brake Pad Diameter (in)" value={data.brake['Maximum Thick']?.padDiameter || ''} onChange={e => handleInputChange('brake', 'padDiameter', e.target.value)} />
            <Input label="Cylinder Bore (in)" value={data.brake['Maximum Thick']?.cylinderBore || ''} onChange={e => handleInputChange('brake', 'cylinderBore', e.target.value)} />
            <Input label="Coefficient of Friction" value={data.brake['Maximum Thick']?.friction || ''} onChange={e => handleInputChange('brake', 'friction', e.target.value)} />
          </div>
        </Card>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <Card className="p-4">
            <Text as="h3" className="mb-4 text-lg font-medium">Threading Drive</Text>
            <Input label="Air Clutch" value={data.threadingDrive['Maximum Thick']?.airClutch || ''} onChange={e => handleInputChange('threadingDrive', 'airClutch', e.target.value)} />
            <Select
              label="Hyd. Threading Drive"
              value={data.threadingDrive['Maximum Thick']?.hydThreadingDrive || ''}
              onChange={e => handleInputChange('threadingDrive', 'hydThreadingDrive', e.target.value)}
              options={HYDRAULIC_THREADING_DRIVE_OPTIONS}
            />
            <Input label="Torque At Mandrel (in. lbs.)" value={data.threadingDrive['Maximum Thick']?.torqueAtMandrel || ''} onChange={e => handleInputChange('threadingDrive', 'torqueAtMandrel', e.target.value)} />
            <Input label="Rewind Torque Req. (in. lbs.)" value={data.threadingDrive['Maximum Thick']?.rewindTorque || ''} onChange={e => handleInputChange('threadingDrive', 'rewindTorque', e.target.value)} />
          </Card>
          <Card className="p-4">
            ...
            <Select
              ...
              onChange={e => handleInputChange('holdDown', 'assy', e.target.value)}
              options={HOLD_DOWN_ASSY_OPTIONS}
            />
            <Input label="Holddown Pressure (psi)" value={data.holdDown['Maximum Thick']?.pressure || ''} onChange={e => handleInputChange('holdDown', 'pressure', e.target.value)} />
            <Input label="Hold Down Force Required (lbs)" value={data.holdDown['Maximum Thick']?.forceRequired || ''} onChange={e => handleInputChange('holdDown', 'forceRequired', e.target.value)} />
            <Input label="Hold Down Force Available (lbs)" value={data.holdDown['Maximum Thick']?.forceAvailable || ''} onChange={e => handleInputChange('holdDown', 'forceAvailable', e.target.value)} />
            <Input label="Min. Material Width (in)" value={data.holdDown['Maximum Thick']?.minWidth || ''} onChange={e => handleInputChange('holdDown', 'minWidth', e.target.value)} />
          </Card>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <Card className="p-4">
            ...
            <Select
              ...
              onChange={e => handleInputChange('cylinder', 'type', e.target.value)}
              options={HOLD_DOWN_CYLINDER_OPTIONS}
            />
            <Input label="Pressure (psi)" value={data.cylinder['Maximum Thick']?.pressure || ''} onChange={e => handleInputChange('cylinder', 'pressure', e.target.value)} />
          </Card>
          <Card className="p-4">
            ...
            <Select
              ...
              onChange={e => handleInputChange('dragBrake', 'reelModel', e.target.value)}
              options={BRAKE_MODEL_OPTIONS}
            />
            <Select
              label="Brake Quantity"
              value={data.dragBrake['Maximum Thick']?.quantity || ''}
              onChange={e => handleInputChange('dragBrake', 'quantity', e.target.value)}
              options={BRAKE_QUANTITY_OPTIONS}
            />
            <Input label="Torque Required (in. lbs.)" value={data.dragBrake['Maximum Thick']?.torqueRequired || ''} onChange={e => handleInputChange('dragBrake', 'torqueRequired', e.target.value)} />
            <Input label="Failsafe - Single Stage (psi air req.)" value={data.dragBrake['Maximum Thick']?.failsafePSI || ''} onChange={e => handleInputChange('dragBrake', 'failsafePSI', e.target.value)} />
            <Input label="Failsafe Holding Force (in. lbs.)" value={data.dragBrake['Maximum Thick']?.failsafeHoldingForce || ''} onChange={e => handleInputChange('dragBrake', 'failsafeHoldingForce', e.target.value)} />
          </Card>
        </div>
      </Card>
