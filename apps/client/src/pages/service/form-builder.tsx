import { ChevronDown, ChevronRight, GripVerticalIcon, Plus, Save, Trash2, X } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { useNavigate, useParams, useSearchParams } from "react-router-dom";

import type { IApiResponse } from "@/utils/types";

import { Button, Input, Modal, PageHeader } from "@/components";
...
function FormBuilder() {
  ...
  const handleDragOverContainer = (e: React.DragEvent) => {
    ...
    if (afterElement) {
      const index = Number.parseInt(afterElement.getAttribute("data-index") || "0");
...
function EditPageView({
  ...
  const handleFieldDragOver = (e: React.DragEvent, sectionId: string) => {
    ...
    if (afterElement) {
      const index = Number.parseInt(afterElement.getAttribute("data-field-index") || "0");
...
function FieldEditor({ field, controlTypes, dataTypes }: any) {
