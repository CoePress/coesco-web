 ...
 */
export const transformDataForAutofill = (data: PerformanceData): any => {
  ...
  const convertToNumber = (value: any, forceInteger = false): any => {
    if (typeof value === 'string' && value.trim() !== '') {
      const parsed = forceInteger ? parseInt(value) : parseFloat(value);
  ...
  const convertNumericFields = (obj: any, path = ''): void => {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (typeof value === 'object' && value !== null) {
        ...
        const stringFields = [
          'typeOfRoll', 'feedDirection', 'materialType', 'grade', 'coating',
          'surfaceCondition', 'edgeCondition', 'typeOfLine', 'passline', 'date',
          'dates.date', 'rfq.dates.date'
        ];
        ...
        const integerFields = [
          'numberOfRolls', 'quantity', 'coilID', 'shiftsPerDay', 'daysPerWeek',
          'rollCount', 'guideQuantity', 'cylinderCount', 'maxCoilOD', 'maxCoilWidth',
          'minCoilWidth', 'maxCoilWeight'
        ];
