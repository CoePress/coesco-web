...
export class InitialAutofillTriggerService {
  // Note: In server context, storage would need to be implemented differently
  // (e.g., using database or cache instead of localStorage)
  private static readonly storageCache = new Map<string, FieldCompletionState>();
   ...
   * (In server context, this would query from database/cache)
   */
  public static getCompletionState(performanceSheetId: string): FieldCompletionState {
    const stored = this.storageCache.get(performanceSheetId);
    if (stored) {
      return stored;
    }
   ...
   * (In server context, this would save to database/cache)
   */
  private static saveCompletionState(performanceSheetId: string, state: FieldCompletionState): void {
    this.storageCache.set(performanceSheetId, state);
  }
  ...
  public static resetCompletionState(performanceSheetId: string): void {
    this.storageCache.delete(performanceSheetId);
  }

  /**
   * Get completion progress for analysis
   */
